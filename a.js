...
var init_config = __esm({
  "src/stuff/config.ts"() {
    config = {
      prefix: `?`,
      prefixes: [`lifestealbackupprefix.`, `?`],
      staffTag: `lifesteal-opped`
    };
  }
});
  ...
  "src/database.ts"() {
    ...
    Database = class {
      ...
      */
       ...
       * @throws Throws RangeError if the string value, key string, or stringify array or object value is more than 32767 characters.
       */
      set(key, value) {
        if (this.localState.get(key) === value)
          return;
        const dynamicPropertyId = `${this.dynamicProperty}:`;
        const allowedKeyLength = 32767 - dynamicPropertyId.length;
        if (key.length > allowedKeyLength)
          throw new RangeError(`Database::set only accepts a key string less than ${allowedKeyLength} characters.`);
        if (typeof value === "string" && value.length > 32767)
          throw new RangeError(`Database::set only accepts a string value less than 32767 characters.`);
        if (typeof value !== "object")
          world.setDynamicProperty(`${dynamicPropertyId}${key}`, value);
          ...
          world.setDynamicProperty(`${dynamicPropertyId}${key}`, stringify);
        } else {
          const newValue = {
            ...value,
...
var lifesteal, killhistory, revivelist, deadplayers, lifestealLogs;
var init_databases = __esm({
  "src/lifesteal/databases.ts"() {
    init_database();
    lifesteal = new Database(`lifesteal`);
    killhistory = new Database(`killhistory`);
    revivelist = new Database(`revivelist`);
    deadplayers = new Database(`deadplayers`);
    lifestealLogs = [];
  }
});
import { system } from "@minecraft/server";
function runLifestealCommand(sender, message) {
  ...
  lifestealLogs.push(`\xA7q${date} > ${sender.name} used - ${message}`);
  ...
  if (!cmdData) {
    sendPlayerMessage(sender, `\xA7cUnknown command: "${command}". Do ${config.prefix}help for a list of valid commands.`);
    return;
  }
  if (cmdData.staffOnly && !sender.hasTag(config.staffTag) && sender.commandPermissionLevel < 2)
    return sendPlayerMessage(sender, `\xA7cYou do not have permission to run this command!`);
  const player = sender;
  if (!cmdData.helpMenu)
...
`).replaceAll(/  +/g, ``) : message;
  if (!requiredTag) {
    world3.sendMessage(formattedMessage);
  } else {
...
function helpMoreInfo(player, args) {
  ...
  if (!cmdData)
    return false;
  if (cmdData.staffOnly && !cmdData.altHelpMenu && player.commandPermissionLevel < 2 && !player.hasTag(config.staffTag)) {
  ...
  if (cmdData.altHelpMenu && player.commandPermissionLevel < 2 && !player.hasTag(config.staffTag)) {
...
function random(length, autoCharacters, characters) {
  if (autoCharacters)
    characters = `ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwyxz0123456789_`;
  let result = ``;
...
function getScore(player, objective) {
  try {
    return world3.scoreboard.getObjective(objective)?.getScore(player.scoreboardIdentity);
  } catch {
  }
  return void 0;
}
function setScore(player, objective, score) {
  return world3.scoreboard.getObjective(objective).setScore(player, score);
}
function addScore(player, objective, amount) {
  return world3.scoreboard.getObjective(objective).addScore(player.scoreboardIdentity, amount);
}
var getPlayer, timeStamp;
var init_commonUtils = __esm({
  "src/stuff/commonUtils.ts"() {
    init_config();
    init_handler();
    getPlayer = (playerName, id, strict = true) => {
      const newPlayerName = playerName?.toLowerCase().trim().replace(/["'@]/g, "");
      const allPlayersArr = world3.getAllPlayers();
      const player = allPlayersArr.find((players) => players.name.toLowerCase() === newPlayerName || !strict && players.name.toLowerCase().startsWith(newPlayerName) || id && players.id === newPlayerName);
      return player ?? null;
    };
    timeStamp = (time) => {
...
var ghostHandling_exports = {};
__export(ghostHandling_exports, {
  processAllDeadplayers: () => processAllDeadplayers,
  processDeadPlayer: () => processDeadPlayer
});
import { world as world4 } from "@minecraft/server";
function processDeadPlayer(player) {
  if (revivelist.has(player.name.toLowerCase())) {
    if (!player.hasTag(`ls-dead`))
      return revivelist.delete(player.name.toLowerCase());
...
To get revived, a player must use a Heart of Life item and revive you`).successCount;
    if (successCount === 0)
      console.warn(`Failed to kick ${player.name}`);
  }
}
function processAllDeadplayers() {
  for (const player of world4.getAllPlayers()) {
    processDeadPlayer(player);
  }
}
var init_ghostHandling = __esm({
  "src/lifesteal/core_modules/ghostHandling.ts"() {
    init_databases();
    init_commonUtils();
  }
});
var lifestealVersion, newFeatures, changelog;
var init_changelog = __esm({
  "src/lifesteal/changelog.ts"() {
    lifestealVersion = `1.18.4`;
    newFeatures = `\xA79${lifestealVersion}\xA7a
\u2022 Heart of Life now works (HOW LONG HAS THIS BEEN BROKEN??)
\u2022 Updated text on the GUI that shows when you get eliminated

\xA791.18.3\xA7a
\u2022 Withdrawn hearts now bypass the heart (item) cap for the player that withdrew them

\xA791.18.2\xA7a
\u2022 Fixed players not gaining hearts after eliminating another player (again)

\xA791.18.0\xA7a
\u2022 Added 1.21.100 support (wowzers 100!!!)
\u2022 Lifesteal no longer uses beta apis
\u2022 Lifesteal commands are now legacy lifesteal commands. Use /ls:console "<command and args>" to run them. e.g /ls:console "withdraw 5"`;
    changelog = `${newFeatures.replaceAll(`\u2022`, `\xA7r\u2022\xA7a`)}

\xA791.17.8\xA7a
\u2022 Added compatibility with Revive Me!
\u2022 Added a tag which prevents stealing hearts when dying (lifesteal:ignore). Use when you want to disable losing/gaining hearts for a specific player or purpose

\xA791.17.7\xA7a
\u2022 Added 1.21.90 support
\u2022 Operators on realms no longer need the staff tag to use staff commands
\u2022 Fixed outdated revive help page

\xA79\xA71.17.6\xA7a
\u2022 Fixed admin panel settings not working
\u2022 Changed "Default: X" on the bottom/top of text when changing settings in the admin panel to a tooltip
...
\xA791.17.4\xA7a
\u2022 New pack icon
\u2022 Removed redundant files. Lower file size

\xA791.17.3\xA7a
\u2022 ?settings now takes you to the admin panel
\u2022 Fixed a bug that caused lifesteal not to work on new worlds
\u2022 Fixed a bug that allowed you to dupe hearts
\u2022 Fixed a bug that caused players to be reset to 10 hearts when joining
\u2022 Fixed a bug that made players with spaces in their name immune to bans on 0 hearts

\xA791.17.2\xA7a
\u2022 Added 1.21.70 (Spring To Life) support

\xA791.17.1\xA7a
\u2022 Fixed using 2 hearts at a time to get 1 heart
\u2022 Fixed hearts not giving you a heart

\xA791.17.0\xA7a
\u2022 Added 1.21.60 support
\u2022 New welcome GUI
\u2022 You are no longer given the "You have been given Heart * 1" message if you are over max hearts and get hearts given to you
\u2022 You are now required to sneak twice within 10s instead of typing "Yes" or "No" when doing ?reviveall
\u2022 ?adminpanel is now more visible in ?help
\u2022 Updated legacy code

\u2022 Added ?heart maxholbonus, which sets the maximum heart bonus you can get from the heart of life

\u2022 Added "Unselected" option to multiple player dropdowns so console players can choose not to select a player
\u2022 - Added to admin panel
\u2022 - Added to revive GUI
\u2022 - Added to ghost panel

\u2022 Changes to admin panel
\u2022 - Added "Max Heart of Life Bonus" module to set max HOL bonus
\u2022 - Added "Heart Item Cap" module to set Heart Item Cap
\u2022 - Added "Environment Drop" module to set Envdrop (Hearts drop on death by natural causes)
\u2022 - Changes to "Revive a player" module
\u2022 - - Toggles have been changed to be "Hide revive" and "Revive all" for more control over whether you want it to show in chat
\u2022 - - Now opens the regular Revive GUI with requireBeacon set to false
\u2022 - It now asks you to confirm before clearing kill history
\u2022 - Improved the "Run multiple commands" module to be able to take infinite inputs. However to make sure the gui isn't the size of your mother this is limited to 30.
\u2022 - Default settings are now shown to you when you reset all settings in the "Reset all settings" module

\u2022 Changes to Revive GUI
\u2022 - No longer uses recursion when searching
\u2022 - You are now asked to confirm before reviving to avoid mistakes

\u2022 Changes to ?heart
\u2022 - Renamed "setmax" to "max". Functionality stays the same
\u2022 - Players without permission are now able to do ?help heart
\u2022 - You can now target players with spaces in their names by surrounding it with double quotes "
\u2022 - Changes to ?heart help page
\u2022 - - Regular help page is no longer indigo
\u2022 - - Staff help page has been updated. Regular ?heart commands are separated to be more visible

\u2022 Other minor changes`;
  }
});
import { CustomCommandParamType as CustomCommandParamType3, CustomCommandStatus as CustomCommandStatus3, ItemStack, system as system6, world as world5 } from "@minecraft/server";
function withdrawAmount(player, amount) {
  if (amount < 1)
    return;
  else if (getScore(player, `ls-death`) <= amount)
    return sendPlayerMessage(player, `\xA7cYou cannot withdraw more hearts than you currently have! (${getScore(player, `ls-death`)})`);
  addScore(player, `ls-death`, -amount);
  const inventory = player.getComponent(`inventory`).container;
  const inventorySize = inventory.size;
  for (let i = 0; i < inventorySize; i++) {
    let item = inventory.getItem(i);
    if (!item || item.typeId !== `lifesteal:heart`)
      continue;
    inventory.setItem(i, new ItemStack(item.typeId, item.amount));
  }
  const sendCommandFeedback = world5.gameRules.sendCommandFeedback;
  ...
  addScore(player, `ls-owed-hearts`, amount);
  sendPlayerMessage(player, `\xA7aWithdrew ${amount} heart(s)`);
}
var init_withdraw = __esm({
  "src/lifesteal/commands/withdraw.ts"() {
    init_commonUtils();
    init_config();
    init_handler();
    SlashCommandBuilder.createCommand({
      commandInfo: {
        ...
        permissionLevel: 0,
        mandatoryParameters: [{ name: `amount`, type: CustomCommandParamType3.Integer }],
        cheatsRequired: false
      },
      callback(origin, amount) {
        const player = origin.sourceEntity;
        if (player?.typeId !== `minecraft:player`)
          return { status: CustomCommandStatus3.Failure, message: `Not a player` };
        if (amount < 1)
          return { status: CustomCommandStatus3.Failure, message: `You cannot withdraw less than 1 heart!` };
        system6.run(() => withdrawAmount(player, amount));
        return { status: CustomCommandStatus3.Success };
      }
    });
    new Command({
      name: `withdraw`,
      helpMenu: `\xA7e\xA7lLifesteal Help Page -- withdraw\xA7r\xA7g
...
> Examples: [
    ...
    ${config.prefix}withdraw 6
      ...
      callback({ player, args, cmdData }) {
        if (!args[0])
          return sendPlayerMessage(player, cmdData.helpMenu);
        const amount = +args[0];
        if (!Number.isInteger(amount))
          return sendPlayerMessage(player, `\xA7cExpected integer ('${args[0]}')`);
        if (amount < 1)
          return sendPlayerMessage(player, `\xA7cYou cannot withdraw less than 1 heart!`);
...
function openWelcomeGui(player) {
  const gui = new ActionFormData().title(`Welcome!`).body(`\xA7gWelcome! This world has the \xA76Lifesteal Addon\xA7g installed!
This lifesteal addon was created by \xA71\xA7oGamerDos\xA7r\xA7g on YouTube.

\xA7lType /ls: in chat to see all slash commands`).button(`Next`, `textures/ui/arrow.png`);
  gui.show(player).then((result) => {
    if (result.canceled && result.cancelationReason === FormCancelationReason.UserBusy)
      return openWelcomeGui(player);
    pageTwo(player);
  });
}
function pageTwo(player) {
  player.playSound(`random.orb`, { pitch: 4 });
  const gui = new ActionFormData().title(`OWNER(S) README`).body(`\xA77If you've installed this addon from a website other than \xA7o\xA7fMCPEDL or ModBay\xA7r\xA77 then you've downloaded my addon from an unofficial source! \xA7fSome people reupload other people's addons without their permission. \xA77Next time, get your addons off of \xA7o\xA7fMCPEDL\xA7r\xA77, where it's more moderated. c:`).button(`Close`, `textures/ui/cancel.png`).button(`Showcasers README`, `textures/blocks/bed_head_top.png`).button(`Lifesteal Changelog`, `textures/items/map_filled.png`);
  gui.show(player).then((result) => {
    switch (result.selection) {
      case 0:
        player.playSound(`random.levelup`);
        sendPlayerMessage(player, `\xA77- dsc.gg/gamerdos: Support Discord Server (Join if you need help or you have a question)
- mcpedl.com/lifesteal-addon: The MCPEDL page, check up on this for updates
- patreon.com/gamerdos: Support what I do!

\xA7r\xA7l\xA7lType /ls: in chat to see all slash commands`);
        break;
      case 1:
        return showcasePage(player);
      case 2:
        return changelogPage(player);
    }
  });
}
function showcasePage(player) {
  player.playSound(`random.orb`, { pitch: 4 });
  const gui = new ActionFormData().title(`README`).body(`\xA79I'd appreciate if you link the MCPEDL page \xA7o\xA7dhttps://mcpedl.com/lifesteal-addon\xA7r\xA79 instead of the direct download to the addon. Not only does it help me out a lot, you also don't have to do through the hassle of updating the download yourself when a new update gets rolled out. Thanks :D`).button(`Back`, `textures/ui/arrow_left.png`);
  gui.show(player).then(() => pageTwo(player));
}
function changelogPage(player) {
  player.playSound(`random.orb`, { pitch: 4 });
  const gui = new ActionFormData().title(`Lifesteal Changelog`).body(`${changelog}`).button(`Back`, `textures/ui/arrow_left.png`);
  gui.show(player).then(() => pageTwo(player));
}
var init_welcome = __esm({
  "src/lifesteal/gui/welcome.ts"() {
    init_changelog();
    init_commonUtils();
  }
});
...
import { ButtonState, EasingType, InputButton, InputPermissionCategory, system as system10 } from "@minecraft/server";
function processSpectators() {
  for (const plr of ghostSpectateMap) {
    const player = getPlayer(plr[0]);
    const target = getPlayer(plr[1]);
    const offset = spectateOffsetMap.get(player.id) ?? 1;
    if (!player || !target) {
      if (player)
        player.camera.clear();
      ghostSpectateMap.delete(plr[0]);
      continue;
    }
    const loc = target.getHeadLocation();
    loc.y += 0.5;
    const movement = player.inputInfo.getMovementVector().y;
    if (offset < 2 && movement < 0) {
      spectateOffsetMap.set(player.id, offset - player.inputInfo.getMovementVector().y / 2);
    }
    if (offset > 0 && movement > 0) {
      spectateOffsetMap.set(player.id, offset - player.inputInfo.getMovementVector().y / 2);
    }
    player.camera.setCamera(`minecraft:free`, {
      location: Vector3Utils.subtract(loc, Vector3Utils.scale(player.getViewDirection(), offset)),
      facingLocation: Vector3Utils.add(loc, player.getViewDirection()),
      easeOptions: { easeType: EasingType.InSine, easeTime: 0.07 }
    });
    player.inputPermissions.setPermissionCategory(InputPermissionCategory.Movement, false);
    if (player.inputInfo.getButtonState(InputButton.Sneak) === ButtonState.Pressed) {
      ghostSpectateMap.delete(player.name);
      spectateOffsetMap.delete(player.id);
      player.inputPermissions.setPermissionCategory(InputPermissionCategory.Movement, true);
      sendPlayerMessage(target, `\xA7cGhost ${player.name} is no longer watching you`);
    }
  }
}
var ghostSpectateMap, spectateOffsetMap;
var init_spectate = __esm({
  "src/lifesteal/commands/ghostsOnly/spectate.ts"() {
    init_lib();
    init_commonUtils();
    init_handler();
    init_config();
    init_databases();
    ghostSpectateMap = /* @__PURE__ */ new Map();
    spectateOffsetMap = /* @__PURE__ */ new Map();
    new Command({
      name: `spectate`,
      helpMenu: `\xA7e\xA7lLifesteal Help Page -- spectate\xA7r\xA7g
> Spectate a player while being a ghost, this won't change your coords

> Usage: ${config.prefix}spectate <playerName>

> Examples: [
    ${config.prefix}spectate GBShadow550
]`,
      async callback({ player, args, cmdData }) {
        if (lifesteal.get(`spectating`) !== `true`)
          return sendPlayerMessage(player, `\xA7cThis command is disabled!`);
        if (!player.hasTag(`ls-dead`))
          return sendPlayerMessage(player, `\xA7cOnly ghosts can run this command!`);
        if (!args[0])
          return sendPlayerMessage(player, `${cmdData.helpMenu}`);
        const target = getPlayer(`${args.join(` `)}`);
        if (!target || target?.hasTag(`ls-dead`)) {
          player.sendMessage(`\xA7cFailed to find player.`);
          player.playSound(`random.break`, { volume: 0.5, pitch: 8 });
          return;
        }
        if (ghostSpectateMap.has(player.name))
          return sendPlayerMessage(player, `\xA7cAlready spectating a player! Sneak to cancel`);
        player.camera.fade({ fadeColor: { red: 0, blue: 0, green: 0 }, fadeTime: { fadeInTime: 0.5, fadeOutTime: 0.5, holdTime: 0.5 } });
        await system10.waitTicks(9);
        ghostSpectateMap.set(player.name, target.name);
        sendPlayerMessage(target, `\xA7cGhost ${player.name} is watching you.`);
        sendPlayerMessage(player, `\xA77Sneak to stop spectating`);
      }
    });
  }
});
...
var init_defaultSettings = __esm({
  "src/lifesteal/defaultSettings.ts"() {
    defaultSettings = {
      maxhearts: `20`,
      envsteal: `true`,
      banghosts: `false`,
...
import { ModalFormData as ModalFormData2, FormCancelationReason as FormCancelationReason3 } from "@minecraft/server-ui";
function setHeartLimit(player) {
  const gui = new ModalFormData2().title(`Heart Limit`).slider(`Set Heart Limit`, 10, 200, { defaultValue: +lifesteal.get(`maxhearts`), tooltip: `Default: ${defaultSettings.maxhearts}` }).submitButton(`Set heart limit`);
  gui.show(player).then((result) => {
    if (result.canceled && result.cancelationReason === FormCancelationReason3.UserClosed) {
      return successGui(player, `Failed to change Heart Limit`, `\xA7cCanceled`, `settings`);
    }
    const formValues = result.formValues;
    lifesteal.set(`maxhearts`, `${formValues[0]}`);
    successGui(player, `Success!`, `\xA7aChanged Heart Limit to \xA7f${formValues[0]}\xA7a!`, `settings`);
    lifestealLogs.push(`\xA7q${getFormattedTime()} > ${player.name} changed the Heart Limit to ${formValues[0]} `);
  });
}
var init_setHeartLimitGui = __esm({
  "src/lifesteal/gui/adminpanel/panelSettings/setHeartLimitGui.ts"() {
    init_databases();
    init_defaultSettings();
    init_adminpanel();
  }
});
import { ModalFormData as ModalFormData3, FormCancelationReason as FormCancelationReason4 } from "@minecraft/server-ui";
function setHeartGain(player) {
  const gui = new ModalFormData3().title(`HeartGL - Gained`).textField(`Set the amount of hearts gained per kill`, `Enter a number here`, { defaultValue: lifesteal.get(`heartgain`), tooltip: `Default: ${defaultSettings.heartgain}` }).submitButton(`Set hearts gained`);
  gui.show(player).then((result) => {
    if (result.canceled && result.cancelationReason === FormCancelationReason4.UserClosed) {
      return successGui(player, `Failed to change HeartGL - Gain`, `\xA7cCanceled`, `settings`);
    }
    const formValues = result.formValues;
    if (isNaN(+formValues[0])) {
      return successGui(player, `Failed to change HeartGL - Gain`, `\xA7cNot a Number`, `settings`);
    }
    lifesteal.set(`heartgain`, `${formValues[0]}`);
    successGui(player, `Success!`, `\xA7aChanged the amount of hearts gained per kill to \xA7f${formValues[0]}\xA7a!`, `settings`);
    lifestealLogs.push(`\xA7q${getFormattedTime()} > ${player.name} changed HeartGL - Gain to ${formValues[0]} `);
  });
}
var init_setHeartGainGui = __esm({
  "src/lifesteal/gui/adminpanel/panelSettings/setHeartGainGui.ts"() {
    init_databases();
    init_defaultSettings();
    init_adminpanel();
  }
});
import { ModalFormData as ModalFormData4, FormCancelationReason as FormCancelationReason5 } from "@minecraft/server-ui";
function setHeartLose(player) {
  const gui = new ModalFormData4().title(`HeartGL - Lose`).textField(`Set the amount of hearts lost per death`, `Enter a number here`, { defaultValue: lifesteal.get(`heartlose`), tooltip: `Default: ${defaultSettings.heartlose}` }).submitButton(`Set hearts lost`);
  gui.show(player).then((result) => {
    if (result.canceled && result.cancelationReason === FormCancelationReason5.UserClosed) {
      return successGui(player, `Failed to change HeartGL - Lose`, `\xA7cCanceled`, `settings`);
    }
    const formValues = result.formValues;
    if (isNaN(+formValues[0])) {
      return successGui(player, `Failed to change HeartGL - Lose`, `\xA7cNot a Number`, `settings`);
    }
    lifesteal.set(`heartlose`, `${formValues[0]}`);
    successGui(player, `Success!`, `\xA7aChanged the amount of hearts lost per death to \xA7f${formValues[0]}\xA7a!`, `settings`);
    lifestealLogs.push(`\xA7q${getFormattedTime()} > ${player.name} changed HeartGL - Lose to ${formValues[0]} `);
  });
}
var init_setHeartLoseGui = __esm({
  "src/lifesteal/gui/adminpanel/panelSettings/setHeartLoseGui.ts"() {
    init_databases();
    init_defaultSettings();
    init_adminpanel();
  }
});
import { ModalFormData as ModalFormData5, FormCancelationReason as FormCancelationReason6 } from "@minecraft/server-ui";
function setBanGhosts(player) {
  const gui = new ModalFormData5().title(`Ban Ghosts`).toggle(`Ban ghosts? \xA77(Players with no hearts)`, { defaultValue: JSON.parse(lifesteal.get(`banghosts`)), tooltip: `Default: ${defaultSettings.banghosts}` }).submitButton(`Set Ban Ghosts`);
  gui.show(player).then((result) => {
    if (result.canceled && result.cancelationReason === FormCancelationReason6.UserClosed) {
      return successGui(player, `Failed to change Ban Ghosts`, `\xA7cCanceled`, `settings`);
    }
    const formValues = result.formValues;
    lifesteal.set(`banghosts`, `${formValues[0]}`);
    processAllDeadplayers();
    successGui(player, `Success!`, `\xA7aChanged Ban Ghosts? to \xA7f${formValues[0]}\xA7a!
\xA77(${formValues[0] === true ? `Players with no hearts left will be banned` : `Players with no hearts left will be in spectator`})`, `settings`);
    lifestealLogs.push(`\xA7q${getFormattedTime()} > ${player.name} changed Ban Ghosts? to ${formValues[0]} `);
  });
}
var init_setBanGhostsGui = __esm({
  "src/lifesteal/gui/adminpanel/panelSettings/setBanGhostsGui.ts"() {
    init_ghostHandling();
    init_databases();
    init_defaultSettings();
    init_adminpanel();
...
import { ModalFormData as ModalFormData6, FormCancelationReason as FormCancelationReason7 } from "@minecraft/server-ui";
function setEnvsteal(player) {
  const gui = new ModalFormData6().title(`Environment Steal`).toggle(`Lose Hearts from Natural Causes? \xA77(Such as fall damage, mobs, etc)`, { defaultValue: JSON.parse(lifesteal.get(`envsteal`)), tooltip: `Default: ${defaultSettings.envsteal}` }).submitButton(`Set Environment Steal`);
  gui.show(player).then((result) => {
    if (result.canceled && result.cancelationReason === FormCancelationReason7.UserClosed) {
      return successGui(player, `Failed to change Environment Steal`, `\xA7cCanceled`, `settings`);
    }
    const formValues = result.formValues;
    lifesteal.set(`envsteal`, `${formValues[0]}`);
    successGui(player, `Success!`, `\xA7aChanged Environment Steal to \xA7f${formValues[0]}\xA7a!`, `settings`);
    lifestealLogs.push(`\xA7q${getFormattedTime()} > ${player.name} changed Environment Steal to ${formValues[0]} `);
  });
}
var init_setEnvstealGui = __esm({
  "src/lifesteal/gui/adminpanel/panelSettings/setEnvstealGui.ts"() {
    init_databases();
    init_defaultSettings();
    init_adminpanel();
  }
});
import { ModalFormData as ModalFormData7, FormCancelationReason as FormCancelationReason8 } from "@minecraft/server-ui";
function setDefaultHearts(player) {
  const gui = new ModalFormData7().title(`Start Amount`).slider(`Set Start Amount`, 5, 15, { defaultValue: +lifesteal.get(`startamount`), tooltip: `Default: ${defaultSettings.startamount}` }).submitButton(`Set Start Amount`);
  gui.show(player).then((result) => {
    if (result.canceled && result.cancelationReason === FormCancelationReason8.UserClosed) {
      return successGui(player, `Failed to change Start Amount`, `\xA7cCanceled`, `settings`);
    }
    const formValues = result.formValues;
    lifesteal.set(`startamount`, `${formValues[0]}`);
    successGui(player, `Success!`, `\xA7aChanged Start Amount to \xA7f${formValues[0]}\xA7a!`, `settings`);
    lifestealLogs.push(`\xA7q${getFormattedTime()} > ${player.name} changed the Start Amount to ${formValues[0]} `);
  });
}
var init_setDefaultHeartsGui = __esm({
  "src/lifesteal/gui/adminpanel/panelSettings/setDefaultHeartsGui.ts"() {
    init_databases();
    init_defaultSettings();
    init_adminpanel();
  }
});
import { ModalFormData as ModalFormData8, FormCancelationReason as FormCancelationReason9 } from "@minecraft/server-ui";
function setHeartsOnRevive(player) {
  const gui = new ModalFormData8().title(`Revive Amount`).slider(`Set Revive Amount`, 1, 15, { defaultValue: +lifesteal.get(`reviveamount`), tooltip: `Default: ${defaultSettings.reviveamount}` }).submitButton(`Set Revive Amount`);
  gui.show(player).then((result) => {
    if (result.canceled && result.cancelationReason === FormCancelationReason9.UserClosed) {
      return successGui(player, `Failed to change Revive Amount`, `\xA7cCanceled`, `settings`);
    }
    const formValues = result.formValues;
    lifesteal.set(`reviveamount`, `${formValues[0]}`);
    successGui(player, `Success!`, `\xA7aChanged Revive Amount to \xA7f${formValues[0]}\xA7a!`, `settings`);
    lifestealLogs.push(`\xA7q${getFormattedTime()} > ${player.name} changed the Revive Amount to ${formValues[0]} `);
  });
}
var init_setHeartsOnReviveGui = __esm({
  "src/lifesteal/gui/adminpanel/panelSettings/setHeartsOnReviveGui.ts"() {
    init_databases();
    init_defaultSettings();
    init_adminpanel();
  }
});
import { ModalFormData as ModalFormData9, FormCancelationReason as FormCancelationReason10 } from "@minecraft/server-ui";
function setGhostBarrier(player) {
  const gui = new ModalFormData9().title(`Ghost Barrier Radius`).textField(`How far ghosts can go before being teleported to spawn`, `Write "disable" to disable the barrier`, {
  ...
  gui.show(player).then((result) => {
    if (result.canceled && result.cancelationReason === FormCancelationReason10.UserClosed) {
      return successGui(player, `Failed to change Ghost Barrier`, `\xA7cCanceled`, `settings`);
    }
    const formValues = result.formValues;
    if (isNaN(+formValues[0]) && formValues[0] !== `disable`) {
      return successGui(player, `Failed to change Ghost Barrier`, `\xA7cNot a Number`, `settings`);
    }
    if (+formValues[0] < 5) {
      return successGui(player, `Failed to change Ghost Barrier`, `\xA7cGhost Barrier Radius can't be less than 5`, `settings`);
    }
    lifesteal.set(`ghostbarrier`, `${formValues[0]}`);
    successGui(player, `Success!`, `\xA7aChanged the Ghost Barrier Radius to \xA7f${formValues[0] === `disable` ? `disabled` : formValues[0]}\xA7a!`, `settings`);
    lifestealLogs.push(`\xA7q${getFormattedTime()} > ${player.name} changed the Ghost Barrier Radius to ${formValues[0]} `);
  });
}
var init_setGhostBarrierGui = __esm({
  "src/lifesteal/gui/adminpanel/panelSettings/setGhostBarrierGui.ts"() {
    init_databases();
    init_defaultSettings();
    init_adminpanel();
  }
});
import { ModalFormData as ModalFormData10, FormCancelationReason as FormCancelationReason11 } from "@minecraft/server-ui";
function toggleSpectating(player) {
  const gui = new ModalFormData10().title(`Ghosts Spectate`).toggle(`Ghosts can spectate players? (This won't reveal coordinates)`, { defaultValue: JSON.parse(lifesteal.get(`spectating`)), tooltip: `Default: ${defaultSettings.spectating}` }).submitButton(`Set Ghosts Spectate`);
  gui.show(player).then((result) => {
    if (result.canceled && result.cancelationReason === FormCancelationReason11.UserClosed) {
      return successGui(player, `Failed to change Ghosts can spectate`, `\xA7cCanceled`, `settings`);
    }
    const formValues = result.formValues;
    lifesteal.set(`spectating`, `${formValues[0]}`);
    successGui(player, `Success!`, `\xA7aChanged Ghosts can spectate players? to \xA7f${formValues[0]}\xA7a!
\xA77(${formValues[0] === true ? `Players with no hearts left can spectate alive players outside of spawn, but won't see their coordinates` : `Players with no hearts left will be stuck at spawn and won't be able to spectate alive players`})`, `settings`);
    lifestealLogs.push(`\xA7q${getFormattedTime()} > ${player.name} changed Ghosts can spectate players? to ${formValues[0]} `);
  });
}
var init_toggleSpectatingGui = __esm({
  "src/lifesteal/gui/adminpanel/panelSettings/toggleSpectatingGui.ts"() {
    init_databases();
    init_defaultSettings();
    init_adminpanel();
  }
});
import { ModalFormData as ModalFormData11, FormCancelationReason as FormCancelationReason12, MessageFormData } from "@minecraft/server-ui";
function resetSettingsGui(player) {
  const gui = new ModalFormData11().title(`Solve to confirm`).textField(`Who was the creator of minecraft>`, `Notch or Herobrine`).submitButton(`Reset all settings`);
  gui.show(player).then((result) => {
    player.playSound(`note.pling`, { pitch: 2, volume: 0.1 });
    const { formValues, cancelationReason, canceled } = result;
    if (result.canceled && result.cancelationReason === FormCancelationReason12.UserClosed) {
      successGui(player, `Failed to reset settings`, `\xA7cCanceled`, `settings`);
      return;
    }
    const text = formValues[0];
    if (text.toLowerCase() !== `notch`) {
      successGui(player, `Failed to reset settings`, `\xA7cWrong name!`, `settings`);
      return;
    }
    const confirm = new MessageFormData().title(`Are you really sure?`).body(`Are you \xA78really \xA7rsure you want to reset all lifesteal settings?`).button1(`No`).button2(`Yes`);
    confirm.show(player).then((result2) => {
      const { selection, canceled: canceled2, cancelationReason: cancelationReason2 } = result2;
      if (result2.canceled && result2.cancelationReason === FormCancelationReason12.UserClosed || selection === 0) {
        successGui(player, `Failed to reset settings`, `\xA7cCanceled`, `settings`);
        return;
      }
      lifesteal.clear();
      for (const key in defaultSettings) {
        if (!lifesteal.has(key)) {
          lifesteal.set(key, defaultSettings[key]);
        }
      }
      successGui(player, `Success!`, `\xA7aReset all lifesteal settings to their default\xA7r\xA77
...
var init_resetSettingsGui = __esm({
  "src/lifesteal/gui/adminpanel/panelSettings/resetSettingsGui.ts"() {
    init_databases();
    init_adminpanel();
    init_defaultSettings();
  }
});
import { FormCancelationReason as FormCancelationReason13, ModalFormData as ModalFormData12 } from "@minecraft/server-ui";
function setMaxHolBonus(player) {
  const gui = new ModalFormData12().title(`Max Heart of Life Bonus`).slider(`Set Max Heart of Life Bonus`, 0, 25, {
    defaultValue: +lifesteal.get(`maxholbonus`),
    tooltip: `Default: ${defaultSettings.maxholbonus}`
  }).submitButton(`Set Max HOL Bonus`);
  gui.show(player).then((result) => {
    if (result.canceled && result.cancelationReason === FormCancelationReason13.UserClosed) {
      return successGui(player, `Failed to change Max Heart of Life Bonus`, `\xA7cCanceled`, `settings`);
    }
    const formValues = result.formValues;
    lifesteal.set(`maxholbonus`, `${formValues[0]}`);
    successGui(player, `Success!`, `\xA7aChanged Max Heart of Life Bonus to \xA7f${formValues[0]}\xA7a!`, `settings`);
    lifestealLogs.push(`\xA7q${getFormattedTime()} > ${player.name} changed the Max HOL Bonus to ${formValues[0]} `);
  });
}
var init_setMaxHolBonus = __esm({
  "src/lifesteal/gui/adminpanel/panelSettings/setMaxHolBonus.ts"() {
    init_defaultSettings();
    init_databases();
    init_adminpanel();
  }
});
import { FormCancelationReason as FormCancelationReason14, ModalFormData as ModalFormData13 } from "@minecraft/server-ui";
function setHeartItemCap(player) {
  const gui = new ModalFormData13().title(`Heart Item Cap`).slider(`The Heart Item Cap is how many hearts you can have before the Heart Item stops working and you have to start killing`, 1, +lifesteal.get(`maxhearts`), { defaultValue: +lifesteal.get(`heartitemcap`), tooltip: `Default: ${defaultSettings.heartitemcap}` }).submitButton(`Set Heart Item Cap`);
  gui.show(player).then((result) => {
    if (result.canceled && result.cancelationReason === FormCancelationReason14.UserClosed) {
      return successGui(player, `Failed to change Heart Item Cap`, `\xA7cCanceled`, `settings`);
    }
    const formValues = result.formValues;
    lifesteal.set(`heartitemcap`, `${formValues[0]}`);
    successGui(player, `Success!`, `\xA7aChanged Heart Item Cap to \xA7f${formValues[0]}\xA7a!`, `settings`);
    lifestealLogs.push(`\xA7q${getFormattedTime()} > ${player.name} changed the Heart Item Cap to ${formValues[0]}`);
  });
}
var init_setHeartItemCap = __esm({
  "src/lifesteal/gui/adminpanel/panelSettings/setHeartItemCap.ts"() {
    init_defaultSettings();
    init_databases();
    init_adminpanel();
  }
...
// src/lifesteal/gui/adminpanel/panelSettings/setEnvdropGui.ts
import { FormCancelationReason as FormCancelationReason15, ModalFormData as ModalFormData14 } from "@minecraft/server-ui";
function setEnvdrop(player) {
  const gui = new ModalFormData14().title(`Environment Drop`).toggle(`Whether or not to drop your heart on the ground if you get killed by natural sources \xA77(Such as fall damage, mobs, etc)`, { defaultValue: JSON.parse(lifesteal.get(`envdrop`)), tooltip: `Default: ${defaultSettings.envdrop}` }).submitButton(`Set Environment Drop`);
  gui.show(player).then((result) => {
    if (result.canceled && result.cancelationReason === FormCancelationReason15.UserClosed) {
      return successGui(player, `Failed to change Environment Drop`, `\xA7cCanceled`, `settings`);
    }
    const formValues = result.formValues;
    lifesteal.set(`envdrop`, `${formValues[0]}`);
    successGui(player, `Success!`, `\xA7aChanged Environment Drop to \xA7f${formValues[0]}\xA7a!`, `settings`);
    lifestealLogs.push(`\xA7q${getFormattedTime()} > ${player.name} changed Environment Drop to ${formValues[0]} `);
  });
}
var init_setEnvdropGui = __esm({
  "src/lifesteal/gui/adminpanel/panelSettings/setEnvdropGui.ts"() {
    init_databases();
    init_defaultSettings();
    init_adminpanel();
...
// src/lifesteal/gui/adminpanel/settings.ts
import { ActionFormData as ActionFormData3 } from "@minecraft/server-ui";
function agLifestealSettings(player) {
  const gui = new ActionFormData3().title(`Lifesteal Settings`).button(`Heart Limit`, `textures/ui/absorption_heart.png`).button(`Hearts gained per Kill`, `textures/ui/heart_flash.png`).button(`Hearts lost per Kill`, `textures/ui/heart_half.png`).button(`Ban Ghosts`, `textures/ui/wither_heart_flash.png`).button(`Drop Hearts from Natural Causes`, `textures/ui/poison_heart.png`).button(`Lose Hearts from Natural Causes`, `textures/ui/poison_heart_flash.png`).button(`Default amount of hearts`, `textures/items/withdraw_heart.png`).button(`Amount of hearts when revived`, `textures/ui/freeze_heart_flash.png`).button(`Radius ghosts can go within spawn`, `textures/ui/icon_map.png`).button(`Spectating`, `textures/items/iron_chestplate.png`).button(`Max Heart of Life Bonus`, `textures/items/heart_of_life.png`).button(`Heart Item Cap`, `textures/ui/heart.png`).button(`Reset all settings`, `textures/ui/refresh.png`).button(`Back`, `textures/ui/back_button_default.png`);
  gui.show(player).then((result) => {
    player.playSound(`note.pling`, { pitch: 2, volume: 0.1 });
    const settingsGui = settingsMap[result.selection];
    if (!settingsGui)
      return adminPanel(player);
    settingsGui(player);
  });
}
var settingsMap;
var init_settings = __esm({
  "src/lifesteal/gui/adminpanel/settings.ts"() {
    init_adminpanel();
    init_setHeartLimitGui();
    init_setHeartGainGui();
    init_setHeartLoseGui();
    init_setBanGhostsGui();
    init_setEnvstealGui();
    init_setDefaultHeartsGui();
    init_setHeartsOnReviveGui();
    init_setGhostBarrierGui();
    init_toggleSpectatingGui();
    init_resetSettingsGui();
    init_setMaxHolBonus();
    init_setHeartItemCap();
    init_setEnvdropGui();
    settingsMap = {
      0: setHeartLimit,
      1: setHeartGain,
...
// src/lifesteal/gui/adminpanel/quickCommands/qcLogs.ts
import { ActionFormData as ActionFormData4 } from "@minecraft/server-ui";
function qcLogs(player) {
  const gui = new ActionFormData4().title(`Logs`).body(`\xA7a(The time before \xA7q>\xA7a is UTC time)
\xA72Note: Logs reset when the world restarts or when the length hits 500
\xA7q${lifestealLogs.join(`
`)}`).button(`Back`, `textures/ui/back_button_default.png`);
  gui.show(player).then(() => {
    player.playSound(`note.pling`, { pitch: 2, volume: 0.1 });
    agQuickCommands(player);
  });
}
var init_qcLogs = __esm({
  "src/lifesteal/gui/adminpanel/quickCommands/qcLogs.ts"() {
    init_databases();
    init_quickCommands();
  }
...
// src/lifesteal/gui/adminpanel/quickCommands/qcSetHearts.ts
import { world as world8, GameMode as GameMode4 } from "@minecraft/server";
import { ModalFormData as ModalFormData15, FormCancelationReason as FormCancelationReason16 } from "@minecraft/server-ui";
function qcSetHearts(player) {
  const players = world8.getAllPlayers();
  const gui = new ModalFormData15().title(`Set a player's heart count`).dropdown(`Target player`, [...players.map((x) => x.name), `Unselected`], { defaultValueIndex: players.length }).slider(`Set their heart count to`, 0, 200, { defaultValue: +lifesteal.get(`startamount`) }).submitButton(`Set Player's Hearts`);
  gui.show(player).then((result) => {
    if (result.canceled && result.cancelationReason === FormCancelationReason16.UserClosed || result.formValues[0] === players.length) {
      return successGui(player, `Failed to set player's heart count`, `\xA7cCanceled`, `quickcommands`);
    }
    ...
    if (target.hasTag(`ls-dead`)) {
      target.setGameMode(GameMode4.Survival);
      target.removeTag(`ls-dead`);
      target.onScreenDisplay.setActionBar(`\xA7bYou have been revived!`);
    }
    processAllDeadplayers();
    successGui(player, `Success!`, `\xA7aSet ${target.name}${target.name.charAt(target.name.length - 1) === `s` ? `'` : `'s`} heart count to ${amount}`, `quickcommands`);
    lifestealLogs.push(`\xA7q${getFormattedTime()} > ${player.name} set ${target.name}'s heart count to ${amount} `);
  });
}
var init_qcSetHearts = __esm({
  "src/lifesteal/gui/adminpanel/quickCommands/qcSetHearts.ts"() {
    init_commonUtils();
    init_databases();
    init_adminpanel();
...
// src/lifesteal/gui/adminpanel/quickCommands/qcResetHearts.ts
import { world as world9, GameMode as GameMode5 } from "@minecraft/server";
import { ModalFormData as ModalFormData16, FormCancelationReason as FormCancelationReason17 } from "@minecraft/server-ui";
function qcResetHearts(player) {
  const players = world9.getAllPlayers();
  const gui = new ModalFormData16().title(`Reset a player's heart count`).dropdown(`Target player`, [...players.map((x) => x.name), `Unselected`], { defaultValueIndex: players.length }).submitButton(`Reset Player's Hearts`);
  gui.show(player).then((result) => {
    if (result.canceled && result.cancelationReason === FormCancelationReason17.UserClosed || result.formValues[0] === players.length) {
      return successGui(player, `Failed to reset player's heart count`, `\xA7cCanceled`, `quickcommands`);
    }
    const target = players[+result.formValues[0]];
    setScore(target, `ls-death`, +lifesteal.get(`startamount`));
    if (target.hasTag(`ls-dead`)) {
      target.setGameMode(GameMode5.Survival);
      target.removeTag(`ls-dead`);
      target.onScreenDisplay.setActionBar(`\xA7bYou have been revived!`);
    }
    processAllDeadplayers();
    successGui(player, `Success!`, `\xA7aReset ${target.name}${target.name.charAt(target.name.length - 1) === `s` ? `'` : `'s`} heart count`, `quickcommands`);
    lifestealLogs.push(`\xA7q${getFormattedTime()} > ${player.name} reset ${target.name}'s heart count`);
  });
}
var init_qcResetHearts = __esm({
  "src/lifesteal/gui/adminpanel/quickCommands/qcResetHearts.ts"() {
    init_commonUtils();
    init_databases();
    init_adminpanel();
...
var init_lib2 = __esm({
  "node_modules/@minecraft/vanilla-data/lib/index.js"() {
    ...
    MinecraftBlockTypes = ((MinecraftBlockTypes2) => {
    ...
    MinecraftCameraPresetsTypes = ((MinecraftCameraPresetsTypes2) => {
    ...
    MinecraftCooldownCategoryTypes = ((MinecraftCooldownCategoryTypes2) => {
    ...
    MinecraftDimensionTypes = ((MinecraftDimensionTypes2) => {
    ...
    MinecraftEffectTypes = ((MinecraftEffectTypes2) => {
    ...
    MinecraftEnchantmentTypes = ((MinecraftEnchantmentTypes2) => {
    ...
    MinecraftEntityTypes = ((MinecraftEntityTypes2) => {
    ...
    MinecraftFeatureTypes = ((MinecraftFeatureTypes2) => {
    ...
    MinecraftItemTypes = ((MinecraftItemTypes2) => {
    ...
    MinecraftPotionEffectTypes = ((MinecraftPotionEffectTypes2) => {
    ...
    MinecraftPotionLiquidTypes = ((MinecraftPotionLiquidTypes2) => {
    ...
    MinecraftPotionModifierTypes = ((MinecraftPotionModifierTypes2) => {
...
// src/lifesteal/gui/reviveGui.ts
import { world as world10 } from "@minecraft/server";
import { ModalFormData as ModalFormData17, FormCancelationReason as FormCancelationReason18, ActionFormData as ActionFormData5 } from "@minecraft/server-ui";
async function askForConfirmation(player, target) {
  const gui = new ActionFormData5().title(`Are you sure?`).body(`Reviving \xA7a\xA7o${target}`).button(`Revive`, `textures/ui/realms_green_check.png`).button(`Back`, `textures/ui/cancel.png`);
...
async function reviveGuiForm(player, requireBeacon = true, hiddenRevive) {
  let options = Array.from(deadplayers.keys());
  let target;
  while (target === void 0) {
    const noOptions = options.length === 0;
    const gui = new ModalFormData17().title(`Revive GUI`).textField(`Search for a player`, `e.g GamerDos7511`).dropdown(`Player to revive`, [...options, `Unselected`], { defaultValueIndex: options.length }).submitButton(`Revive`);
    await gui.show(player).then(async (result) => {
      if (result.canceled && result.cancelationReason === FormCancelationReason18.UserBusy)
        return;
      if (result.canceled && result.cancelationReason === FormCancelationReason18.UserClosed || result.formValues[1] === options.length && !result.formValues[0])
        return target = null;
      const textField = result.formValues[0];
      const dropdown = result.formValues[1];
      const deadPlayersList = Array.from(deadplayers.keys());
      if (textField) {
        options = deadPlayersList.filter(
          (x) => x.includes(textField)
        );
        return;
      }
      if (noOptions)
  ...
  if (requireBeacon && beaconMap.has(player.id)) {
    ...
    if (block.typeId !== `lifesteal:beacon_of_life`)
      return;
    block.setType(MinecraftBlockTypes.Air);
  } else if (requireBeacon && player.runCommand(`clear @s lifesteal:beacon_of_life`).successCount <= 0 && player.commandPermissionLevel < 2)
    return;
  ...
  processAllDeadplayers();
  if (hiddenRevive) {
    sendPlayerMessage(player, `\xA7bRevived ${target}`);
    return target;
  }
  sendAllMessage2(`\xA7b${player.name} has revived ${target}`);
  for (const plr of world10.getAllPlayers()) {
    plr.playSound(`heart_of_life_sound`);
  }
  return target;
}
var beaconMap, beaconDebounceMap;
var init_reviveGui = __esm({
  "src/lifesteal/gui/reviveGui.ts"() {
    init_lib2();
    init_commonUtils();
    init_ghostHandling();
    init_databases();
    beaconMap = /* @__PURE__ */ new Map();
    beaconDebounceMap = /* @__PURE__ */ new Map();
  }
});

// src/lifesteal/gui/adminpanel/quickCommands/qcRevivePlayer.ts
import { world as world11 } from "@minecraft/server";
import { ModalFormData as ModalFormData18, FormCancelationReason as FormCancelationReason19 } from "@minecraft/server-ui";
function qcRevivePlayer(player) {
  const gui = new ModalFormData18().title(`Revive options`).toggle(`Hide Revive (No message or sound)`).toggle(`Revive all`);
  gui.show(player).then(async (result) => {
    if (result.canceled && result.cancelationReason === FormCancelationReason19.UserClosed)
      return successGui(player, `Failed to revive`, `\xA7cCanceled`, `quickcommands`);
    const hideRevive = result.formValues[0];
    const reviveAll = result.formValues[1];
    if (!reviveAll) {
      player.playSound(`note.pling`, { pitch: 2, volume: 0.1 });
      const revivedPlayer = await reviveGuiForm(player, false, hideRevive);
      if (revivedPlayer)
        successGui(player, `Success!`, `\xA7aRevived ${revivedPlayer}`, `quickcommands`);
      else
        successGui(player, `Failed to revive player`, `\xA7cCanceled`, `quickcommands`);
      return;
    }
    ...
    processAllDeadplayers();
    if (hideRevive)
      return successGui(player, `Success!`, `\xA7aRevived all players`, `quickcommands`);
    sendAllMessage2(`\xA7b${player.name} has revived all players`);
    for (const player2 of world11.getAllPlayers()) {
      player2.playSound(`heart_of_life_sound`);
    }
    successGui(player, `Success!`, `\xA7aRevived all players`, `quickcommands`);
  });
}
var init_qcRevivePlayer = __esm({
  "src/lifesteal/gui/adminpanel/quickCommands/qcRevivePlayer.ts"() {
    init_commonUtils();
    init_databases();
    init_reviveGui();
...
// src/lifesteal/gui/adminpanel/quickCommands/qcGetItem.ts
import { ItemStack as ItemStack2 } from "@minecraft/server";
import { ModalFormData as ModalFormData19, FormCancelationReason as FormCancelationReason20 } from "@minecraft/server-ui";
function qcGetItem(player) {
  const options = [`Heart`, `Heart Fragment`, `Heart Essence`, `Beacon of Life`, `Heart of Life`, `Withdraw Heart`];
  const gui = new ModalFormData19().title(`Get lifesteal item`).dropdown(`Choose which item you want to get:`, options).slider(`Amount`, 1, 64, { defaultValue: 64 }).submitButton(`Get item(s)`);
  gui.show(player).then((result) => {
    if (result.canceled && result.cancelationReason === FormCancelationReason20.UserClosed)
      return successGui(player, `Failed to get item`, `\xA7cCanceled`, `quickcommands`);
    const inventory = player.getComponent(`inventory`);
    const itemIndex = +result.formValues[0];
    const amount = +result.formValues[1];
    switch (itemIndex) {
      default:
        successGui(player, `Failed to get item`, `\xA7cError`, `quickcommands`);
        break;
      case 0:
        inventory.container.addItem(new ItemStack2(`lifesteal:heart`, amount));
        successGui(player, `Success!`, `\xA7aGave yourself \xA7f${amount} Heart(s)`, `quickcommands`);
        lifestealLogs.push(`\xA7q${getFormattedTime()} > ${player.name} gave themselves ${amount} heart(s)`);
        break;
      case 1:
        inventory.container.addItem(new ItemStack2(`lifesteal:heart_fragment`, amount));
        successGui(player, `Success!`, `\xA7aGave yourself \xA7f${amount} Heart Fragment(s)`, `quickcommands`);
        lifestealLogs.push(`\xA7q${getFormattedTime()} > ${player.name} gave themselves ${amount} heart fragment(s)`);
        break;
      case 2:
        inventory.container.addItem(new ItemStack2(`lifesteal:heart_essence`, amount));
        successGui(player, `Success!`, `\xA7aGave yourself \xA7f${amount} Heart Essence(s)`, `quickcommands`);
        lifestealLogs.push(`\xA7q${getFormattedTime()} > ${player.name} gave themselves ${amount} heart essence(s)`);
        break;
      case 3:
        inventory.container.addItem(new ItemStack2(`lifesteal:beacon_of_life`, amount));
        successGui(player, `Success!`, `\xA7aGave yourself \xA7f${amount} Beacon of Life(s)`, `quickcommands`);
        lifestealLogs.push(`\xA7q${getFormattedTime()} > ${player.name} gave themselves ${amount} beacon of life(s)`);
        break;
      case 4:
        inventory.container.addItem(new ItemStack2(`lifesteal:heart_of_life`, amount));
        successGui(player, `Success!`, `\xA7aGave yourself \xA7f${amount} Heart of Life(s)`, `quickcommands`);
        lifestealLogs.push(`\xA7q${getFormattedTime()} > ${player.name} gave themselves ${amount} heart of life(s)`);
        break;
      case 5:
        inventory.container.addItem(new ItemStack2(`lifesteal:withdraw_heart`, amount));
        successGui(player, `Success!`, `\xA7aGave yourself \xA7f${amount} Withdraw Heart(s)`, `quickcommands`);
        lifestealLogs.push(`\xA7q${getFormattedTime()} > ${player.name} gave themselves ${amount} withdraw heart(s)`);
        break;
    }
  });
}
var init_qcGetItem = __esm({
  "src/lifesteal/gui/adminpanel/quickCommands/qcGetItem.ts"() {
    init_databases();
    init_adminpanel();
  }
...
// src/lifesteal/gui/adminpanel/quickCommands/qcRunCommands.ts
import { world as world12 } from "@minecraft/server";
import { ModalFormData as ModalFormData20, FormCancelationReason as FormCancelationReason21 } from "@minecraft/server-ui";
function qcRunCommands(player) {
  const gui = new ModalFormData20().title(`Run commands`).slider(`How many commands would you like to run?`, 1, 30, { defaultValue: 3 }).textField(`Or enter an integer here (1-30)`, `Uses slider if invalid or < 1`);
  gui.show(player).then((result) => {
    if (result.canceled && result.cancelationReason === FormCancelationReason21.UserClosed)
      return successGui(player, `Failed to run commands`, `\xA7cCanceled`, `quickcommands`);
    const slider = result.formValues[0];
    const timesToRun = Number.isInteger(+result.formValues[1]) && +result.formValues[1] > 0 ? +result.formValues[1] : slider;
    runCommandsGui(player, timesToRun > 30 ? 30 : timesToRun);
  });
}
function runCommandsGui(player, inputFieldCount) {
  player.playSound(`note.pling`, { pitch: 2, volume: 0.1 });
  const players = [``].concat(world12.getAllPlayers().map((x) => x.name));
  const gui = new ModalFormData20().title(`Run commands`);
  ...
  gui.dropdown(`Executor:`, [player.dimension.id, ...players.slice(1)]).submitButton(`Run`);
  gui.show(player).then((result) => {
    if (result.canceled && result.cancelationReason === FormCancelationReason21.UserClosed) {
    ...
    const guiOutput = successes.map((v, i) => `\xA7rInput ${i + 1}: ${v}`);
    const logsOutput = commmandsToRun.map((v, i) => `\xA7r/${v} - \xA77${successes[i]}\xA7r`).filter((_, i) => commmandsToRun[i]);
    successGui(player, `Ran commands`, `${guiOutput.join(`
`)}`, `quickcommands`);
    lifestealLogs.push(`\xA7q${getFormattedTime()} > ${player.name} ran commands using the Admin Panel as ${executorIndex === 0 ? executor.id : executor.name}:
\xA7o${logsOutput.length === 0 ? `\xA77No commands executed\xA7r` : logsOutput.join(`
`)}\xA7r\xA7g`);
  });
}
function runCommands(executor, commands2, timesToRun) {
  const successes = [];
  for (let i = 0; i < commands2.length; i++) {
    if (!commands2[i]) {
      successes[i] = `\xA77No command given\xA7r`;
      continue;
    }
    let commandSuccessCount = 0;
    for (let current = 0; current < timesToRun[i]; current++) {
      try {
        commandSuccessCount += executor.runCommand(commands2[i]).successCount;
      } catch (error) {
        successes[i] = `\xA7c${error}\xA7r`;
        break;
      }
    }
    if (!successes[i])
      successes[i] = `\xA7aRan ${commandSuccessCount} times\xA7r`;
  }
  return successes;
}
var init_qcRunCommands = __esm({
  "src/lifesteal/gui/adminpanel/quickCommands/qcRunCommands.ts"() {
    init_commonUtils();
    init_databases();
    init_adminpanel();
...
// src/lifesteal/gui/adminpanel/quickCommands.ts
import { ActionFormData as ActionFormData6 } from "@minecraft/server-ui";
function agQuickCommands(player) {
  const gui = new ActionFormData6().title(`Quick Commands`).button(`Lifesteal Logs`, `textures/ui/icon_book_writable.png`).button(`Set a player's heart count`, `textures/ui/heart.png`).button(`Reset a player's heart count`, `textures/ui/poison_heart.png`).button(`Revive a player`, `textures/ui/freeze_heart.png`).button(`Get a lifesteal item`, `textures/items/heart_essence.png`).button(`Run multiple commands`, `textures/blocks/command_block.png`).button(`Back`, `textures/ui/back_button_default.png`);
  gui.show(player).then((result) => {
    player.playSound(`note.pling`, { pitch: 2, volume: 0.1 });
    const quickCommandGui = quickCommandsMap[result.selection];
    if (!quickCommandGui)
      return adminPanel(player);
    quickCommandGui(player);
  });
}
var quickCommandsMap;
var init_quickCommands = __esm({
  "src/lifesteal/gui/adminpanel/quickCommands.ts"() {
    init_adminpanel();
    init_qcLogs();
    init_qcSetHearts();
...
// src/lifesteal/gui/adminpanel/killHistory/clearHistory.ts
import { ActionFormData as ActionFormData7 } from "@minecraft/server-ui";
function clearHistory(player) {
  const gui = new ActionFormData7().title(`Are you sure?`).body(`Are you sure you want to delete all kill history?`).button(`Yes`, `textures/ui/realms_green_check.png`).button(`Back`, `textures/ui/cancel.png`);
  gui.show(player).then((result) => {
    ...
    killhistory.delete(`killhistory`);
    successGui(player, `Success!`, `\xA7aCleared history!`, `killhistory`);
  });
}
var init_clearHistory = __esm({
  "src/lifesteal/gui/adminpanel/killHistory/clearHistory.ts"() {
    init_databases();
    init_adminpanel();
    init_killhistory();
...
// src/lifesteal/gui/adminpanel/killHistory/textView.ts
import { ActionFormData as ActionFormData8 } from "@minecraft/server-ui";
function textView(player) {
  const killHistoryString = killhistory.get(`killhistory`);
  const killHistoryArray = killHistoryString?.split(`,`).reverse() ?? [];
  const gui = new ActionFormData8().title(`Text View`).body(`\xA7q${killHistoryString ? killHistoryArray.map((x) => {
    const v = x.split(`..`);
    return `${v[0]} got killed by ${v[1]} ${timeStamp(Date.now() - +v[2])} ago`;
  }).join(`

`) : `\xA78nothing here`}`).button(`Back`, `textures/ui/back_button_default.png`);
  gui.show(player).then(() => {
    player.playSound(`note.pling`, { pitch: 2, volume: 0.1 });
    agKillHistory(player);
  });
...
// src/lifesteal/gui/adminpanel/killHistory/guiView.ts
import { ActionFormData as ActionFormData9, FormCancelationReason as FormCancelationReason22 } from "@minecraft/server-ui";
function guiView(player) {
  const killHistoryString = killhistory.get(`killhistory`);
  const killHistoryArray = killHistoryString?.split(`,`).reverse() ?? [];
  const gui = new ActionFormData9().title(`Gui View`).button(`Back`, `textures/ui/back_button_default.png`);
  if (!killHistoryArray.length)
    gui.body(`\xA78but nobody came`);
...
${timeStamp(Date.now() - +name[2])} ago`, `textures/ui/icon_steve.png`);
  ...
  gui.show(player).then((result) => {
    player.playSound(`note.pling`, { pitch: 2, volume: 0.1 });
    if (result.canceled && result.cancelationReason === FormCancelationReason22.UserClosed)
      return agKillHistory(player);
    if (result.selection === 0)
      return agKillHistory(player);
    const plr = killHistoryArray[result.selection - 1].split(`..`);
    const khGui = new ActionFormData9().title(`${plr[0]}`).body(`\xA77Killed: ${timeStamp(Date.now() - +plr[2])} ago
Killer: ${plr[1]}`).button(`Back to Kill History`, `textures/ui/back_button_default.png`);
    khGui.show(player).then((result2) => {
      player.playSound(`note.pling`, { pitch: 2, volume: 0.1 });
      if (result2.canceled && result2.cancelationReason === FormCancelationReason22.UserClosed) {
        return successGui(player, `Failed to execute action`, `\xA7cCanceled`, `killhistory`);
      }
      agKillHistory(player);
    });
  });
}
var init_guiView = __esm({
  "src/lifesteal/gui/adminpanel/killHistory/guiView.ts"() {
    init_commonUtils();
    init_databases();
    init_adminpanel();
...
// src/lifesteal/gui/adminpanel/killhistory.ts
import { ActionFormData as ActionFormData10, FormCancelationReason as FormCancelationReason23 } from "@minecraft/server-ui";
function agKillHistory(player) {
  const gui = new ActionFormData10().title(`Kill History`).body(`\xA77Kill history`).button(`Clear history`, `textures/items/iron_shovel.png`).button(`Text view`, `textures/ui/bamboo_sign.png`).button(`GUI view`, `textures/ui/comment.png`).button(`Back`, `textures/ui/back_button_default.png`);
  gui.show(player).then((result) => {
    player.playSound(`note.pling`, { pitch: 2, volume: 0.1 });
    if (result.canceled && result.cancelationReason === FormCancelationReason23.UserClosed)
      return adminPanel(player);
    const killHistoryGui = killHistoryMap[result.selection];
    if (!killHistoryGui)
      return adminPanel(player);
    killHistoryGui(player);
  });
}
var killHistoryMap;
var init_killhistory = __esm({
  "src/lifesteal/gui/adminpanel/killhistory.ts"() {
    init_adminpanel();
    init_clearHistory();
    init_textView();
...
// src/lifesteal/gui/adminpanel/changelogGui.ts
import { ActionFormData as ActionFormData11 } from "@minecraft/server-ui";
function agChangelog(player) {
  const gui = new ActionFormData11().title(`Lifesteal Changelogs`).body(`\xA79Current Version - ${lifestealVersion}

-- Changelogs:
${changelog}`).button(`Back`, `textures/ui/back_button_default.png`);
  gui.show(player).then(() => {
    player.playSound(`note.pling`, { pitch: 2, volume: 0.1 });
    adminPanel(player);
  });
}
var init_changelogGui = __esm({
  "src/lifesteal/gui/adminpanel/changelogGui.ts"() {
    init_changelog();
    init_adminpanel();
  }
...
// src/lifesteal/gui/adminpanel.ts
import { ActionFormData as ActionFormData12, FormCancelationReason as FormCancelationReason24, uiManager as uiManager2 } from "@minecraft/server-ui";
function adminPanel(player) {
  const agMain = new ActionFormData12().title(`Admin Panel`).button(`Lifesteal Settings`, `textures/ui/gear.png`).button(`Quick Commands`, `textures/ui/op.png`).button(`Kill History`, `textures/ui/WarningGlyph.png`).button(`Lifesteal Changelogs`, `textures/items/map_filled.png`).button(`Close`, `textures/ui/cancel.png`);
  agMain.show(player).then((result) => {
    uiManager2.closeAllForms(player);
    if (result.canceled && result.cancelationReason === FormCancelationReason24.UserBusy)
      return adminPanel(player);
    player.playSound(`note.pling`, { pitch: 2, volume: 0.1 });
    switch (result.selection) {
      default:
        sendPlayerMessage(player, `\xA73Admin Panel Closed`);
        break;
      case 0:
        agLifestealSettings(player);
        break;
      case 1:
        agQuickCommands(player);
        break;
...
function successGui(player, guiName, guiMessage, guiToReturnTo) {
  player.playSound(`note.pling`, { pitch: 2, volume: 0.1 });
  const gui = new ActionFormData12().title(guiName).body(guiMessage).button(`Back to Admin Panel`, `textures/ui/back_button_default.png`).button(`Close Admin Panel`, `textures/ui/cancel.png`);
  gui.show(player).then((result) => {
    player.playSound(`note.pling`, { pitch: 2, volume: 0.1 });
    if (result.selection === 1)
      return sendPlayerMessage(player, `\xA73Admin Panel Closed`);
    switch (guiToReturnTo) {
      default:
        throw new Error(`No gui found (${guiToReturnTo})`);
      case `main`:
        adminPanel(player);
        break;
      case `settings`:
...
function getFormattedTime() {
  const now = /* @__PURE__ */ new Date(), hours = ((now.getUTCHours() % 12 || 12) + ``).padStart(2, `0`);
  const minutes = (now.getUTCMinutes() + ``).padStart(2, `0`);
  const amorpm = now.getUTCHours() < 12 ? `AM` : `PM`;
  const date = `${hours}:${minutes} ${amorpm}`;
  return date;
}
var init_adminpanel = __esm({
  "src/lifesteal/gui/adminpanel.ts"() {
    init_commonUtils();
    init_settings();
    init_quickCommands();
...
// src/lifesteal/events/entityHurt.ts
var entityHurt_exports = {};
__export(entityHurt_exports, {
  targetTracker: () => targetTracker
});
import { Player as Player26, system as system18, world as world17 } from "@minecraft/server";
var targetTracker;
var init_entityHurt = __esm({
  "src/lifesteal/events/entityHurt.ts"() {
    world17.afterEvents.entityHurt.subscribe((data) => {
      const { hurtEntity, damageSource } = data;
      const damagingEntity = damageSource.damagingEntity;
      if (!(hurtEntity instanceof Player26) && !(damagingEntity instanceof Player26))
        return;
      targetTracker.set(hurtEntity.id, { tick: system18.currentTick, damagingEntity });
    });
    targetTracker = /* @__PURE__ */ new Map();
  }
});

// src/lifesteal/events/entityDie.ts
var entityDie_exports = {};
import { ItemStack as ItemStack4, system as system19, world as world18 } from "@minecraft/server";
function getEntityTarget(entity) {
  const target = targetTracker.get(entity.id);
  if (target && system19.currentTick - target.tick > 100) {
...
var init_entityDie = __esm({
  "src/lifesteal/events/entityDie.ts"() {
    ...
    init_commonUtils();
    init_entityHurt();
    world18.afterEvents.entityDie.subscribe((data) => {
      const player = data.deadEntity;
      if (player.hasTag(`lifesteal:ignore`))
        return;
      const target = getEntityTarget(player)?.damagingEntity ?? data.damageSource?.damagingEntity;
      const score = getScore(player, `ls-death`);
      const now = /* @__PURE__ */ new Date(), hours = ((now.getUTCHours() % 12 || 12) + ``).padStart(2, `0`);
      const minutes = (now.getUTCMinutes() + ``).padStart(2, `0`);
      const amorpm = now.getUTCHours() < 12 ? `AM` : `PM`;
      const date = `${hours}:${minutes} ${amorpm}`;
      const mode = lifesteal.get(`lifestealmode`);
      let heartsToAdd = +lifesteal.get(`heartgain`);
      const heartLostOnDeath = lifesteal.get(`heartlose`);
      if (mode.includes(`random`))
        heartsToAdd = randomN(+mode.split(`-`)[1], +mode.split(`-`)[2]);
      if (score - heartLostOnDeath === 0)
        sendAllMessage2(`\xA7c${player.name} lost all their hearts`);
      if (!player.hasTag(`heart-notify-off`) && !player.hasTag(`ls-message-debounce`)) {
        if (lifesteal.get(`envsteal`) === `false` && !target)
          return;
        if (heartLostOnDeath < 0) {
          const absheartLostOnDeath = Math.abs(+heartLostOnDeath);
          sendPlayerMessage(player, `\xA7aYou gained ${absheartLostOnDeath === 1 ? `a` : `${absheartLostOnDeath}`} ${absheartLostOnDeath === 1 ? `heart` : `hearts`}. (You now have \xA7f${score + absheartLostOnDeath}\xA7a hearts.)`);
        } else {
          sendPlayerMessage(player, `\xA7cYou lost ${+heartLostOnDeath === 1 ? `a` : `${heartLostOnDeath}`} ${+heartLostOnDeath === 1 ? `heart` : `hearts`}. (You now have \xA76${score - heartLostOnDeath}\xA7c hearts.)`);
        }
      }
      ...
      if (player.typeId === `minecraft:player`) {
        ...
        if (killhistory.get(`killhistory`).length > 1e4) {
          killhistory.clear();
        }
      }
      const envsteal = lifesteal.get(`envsteal`);
      const envdrop = lifesteal.get(`envdrop`);
      if (envsteal === `false` && (!target || target.typeId !== `minecraft:player` || player.typeId !== `minecraft:player`))
        return;
      const hearts = score;
      const lowerThanLost = hearts < heartLostOnDeath;
      addScore(player, `ls-death`, -+heartLostOnDeath);
      lifestealLogs.push(`\xA7q${date} > ${player.name} lost ${heartLostOnDeath} heart(s)`);
      if (!target || target.typeId !== `minecraft:player` || player === target) {
        if (envdrop === `true`) {
          player.dimension.spawnItem(new ItemStack4(`lifesteal:heart`, heartsToAdd), player.location);
        }
        return;
      }
      if (lowerThanLost === true) {
        addScore(player, `ls-death`, hearts);
        lifestealLogs.push(`\xA7q${date} > ${target.name} gained ${hearts} heart(s) from killing ${player.name}`);
        return;
      }
      addScore(target, `ls-death`, heartsToAdd);
      lifestealLogs.push(`\xA7q${date} > ${target.name} gained ${heartsToAdd} heart(s) from killing ${player.name}`);
    }, { entityTypes: [`minecraft:player`] });
  }
});

// src/lifesteal/items/heart_of_life.ts
function heartOfLife(player) {
  const maxHolBonus = +lifesteal.get(`maxholbonus`);
  const currentHolBonus = getScore(player, `ls-extra-hearts`) ?? 0;
  if (currentHolBonus < maxHolBonus && player.runCommand(`clear @s lifesteal:heart_of_life 0 1`).successCount !== 0) {
    addScore(player, `ls-extra-hearts`, 1);
    sendPlayerMessage(player, `\xA7bYou will now recieve one extra heart when getting revived (Permanently)`);
  }
}
var init_heart_of_life = __esm({
  "src/lifesteal/items/heart_of_life.ts"() {
    init_commonUtils();
    init_databases();
  }
...
// src/lifesteal/items/withdraw_heart.ts
import { ModalFormData as ModalFormData21 } from "@minecraft/server-ui";
function withdrawHeart(player, item) {
  if (item.typeId !== `lifesteal:withdraw_heart`)
    return;
  if (player.isSneaking)
    return withdrawAmount(player, 1);
  withdrawGUI(player);
}
function withdrawGUI(player) {
  const heartCount2 = getScore(player, `ls-death`);
  const gui = new ModalFormData21().title(`Withdraw GUI`).label(`\xA77Sneaking while using the withdraw heart quick withdraws a heart.`).slider(`Select how many hearts to withdraw`, 0, heartCount2 - 1).textField(`Or type an amount here`, `${heartCount2 - 1}`).submitButton(`Withdraw`);
  gui.show(player).then((result) => {
    if (result.canceled)
      return;
    const sliderAmount = result.formValues[1];
...
// src/lifesteal/items/heart.ts
function heartItem(player, item) {
  if (item.typeId !== `lifesteal:heart`)
    return;
  const now = /* @__PURE__ */ new Date(), hours = ((now.getUTCHours() % 12 || 12) + ``).padStart(2, `0`);
  const minutes = (now.getUTCMinutes() + ``).padStart(2, `0`);
  const amorpm = now.getUTCHours() < 12 ? `AM` : `PM`;
  const date = `${hours}:${minutes} ${amorpm}`;
  const itemCap = +lifesteal.get(`heartitemcap`);
  const hearts = getScore(player, `ls-death`);
  const owedHearts = getScore(player, `ls-owed-hearts`) ?? 0;
  if (getScore(player, `ls-death`) >= 200) {
    sendPlayerMessage(player, `\xA7cYou are at the max amount of hearts!`);
    player.playSound(`note.bd`, { pitch: 1.2, volume: 0.5 });
    setScore(player, `ls-nomsg`, 7);
    return;
  }
  if (getScore(player, `ls-death`) >= +lifesteal.get(`maxhearts`)) {
    sendPlayerMessage(player, `\xA7cYou are at the max amount of hearts!`);
    player.playSound(`note.bd`, { pitch: 1.2, volume: 0.5 });
    setScore(player, `ls-nomsg`, 7);
    return;
  }
  if (hearts >= itemCap && owedHearts === 0) {
    player.playSound(`note.bd`, { pitch: 1.2, volume: 0.5 });
    sendPlayerMessage(player, `\xA7cYou've reached the heart (item form) limit! Kill other players to gain hearts. \xA78\xA7oUse the admin panel by doing \xA7l/ls:adminpanel\xA7r\xA78 to change the limit`);
    return;
  }
  if (player.runCommand(`clear @s lifesteal:heart 0 1`).successCount == 0)
    return;
  addScore(player, `ls-death`, 1);
  if (owedHearts > 0)
    addScore(player, `ls-owed-hearts`, -1);
  player.playSound(`heart`, { volume: 0.3 });
  lifestealLogs.push(`\xA7q${date} > ${player.name} gained a heart by using a heart item`);
}
...
// src/lifesteal/items/iadminpanel.ts
function adminPanelItem(player) {
  if (player.hasTag(`lifesteal-opped`) || player.commandPermissionLevel >= 2) {
    adminPanel(player);
    lifestealLogs.push(`\xA7q${getFormattedTime2()} > ${player.name} opened the Admin Panel`);
  } else {
    sendPlayerMessage(player, `\xA7cYou do not have permission to open the Admin Panel! Do /tag @s add lifesteal-opped to get permission`);
    lifestealLogs.push(`\xA7q${getFormattedTime2()} > ${player.name} tried opened the Admin Panel, but they weren't opped`);
  }
}
function getFormattedTime2() {
  const now = /* @__PURE__ */ new Date(), hours = ((now.getUTCHours() % 12 || 12) + ``).padStart(2, `0`);
  const minutes = (now.getUTCMinutes() + ``).padStart(2, `0`);
  const amorpm = now.getUTCHours() < 12 ? `AM` : `PM`;
  const date = `${hours}:${minutes} ${amorpm}`;
...
// src/lifesteal/events/itemUse.ts
var itemUse_exports = {};
...
var init_itemUse = __esm({
  "src/lifesteal/events/itemUse.ts"() {
    ...
    init_heart();
    init_iadminpanel();
    world19.afterEvents.itemUse.subscribe((data) => {
      const { itemStack: item, source: player } = data;
      switch (item.typeId) {
        case `lifesteal:heart_of_life`:
          return heartOfLife(player);
        case `lifesteal:withdraw_heart`:
          return withdrawHeart(player, item);
        case `lifesteal:heart`:
          return heartItem(player, item);
        case `lifesteal:admin_panel`:
          return adminPanelItem(player);
      }
    });
  }
});

// src/lifesteal/events/playerSpawn.ts
var playerSpawn_exports = {};
import { system as system20, world as world20 } from "@minecraft/server";
import { ActionFormData as ActionFormData13, FormCancelationReason as FormCancelationReason25 } from "@minecraft/server-ui";
var init_playerSpawn = __esm({
  "src/lifesteal/events/playerSpawn.ts"() {
    ...
    world20.afterEvents.playerSpawn.subscribe(({ player, initialSpawn }) => {
      if (player.hasTag(`ls-dead`)) {
        let ghostWelcomeForm = function() {
          if (initialSpawn)
            return;
          const ghostWelcome = new ActionFormData13().title(`You died!`).body(`\xA7gYou have lost your last heart and have become a ghost!

\xA7bAnother player must use a Beacon of Life to revive you`).button(`Alright`);
          ghostWelcome.show(player).then((result) => {
            if (result.canceled && result.cancelationReason == `UserBusy`)
              return ghostWelcomeForm();
          });
        };
        player.camera.clear();
        processAllDeadplayers();
      ...
      if (player.hasTag(config.staffTag) && player.hasTag(`welcome-gui_done`) && !player.hasTag(`lsversion-${lifestealVersion}`)) {
        let newFeaturesFunction = function() {
          const gui = new ActionFormData13().title(`Lifesteal Changelog`).body(`${newFeatures.replaceAll(`\u2022`, `\xA7r\u2022\xA7a`)}`).button(`Ok`);
          gui.show(player).then((result) => {
            if (result.canceled && result.cancelationReason === FormCancelationReason25.UserBusy)
              newFeaturesFunction();
          });
        };
      ...
      system20.runTimeout(() => {
        ...
        if (!eventCheck && lifesteal.get(`useplayerjson`) === `true` && !world20.getDynamicProperty(`lifesteal:ignore-join-error`)) {
          sendAllMessage2(`\xA7cAn error occured while loading lifesteal. Things that might have went wrong:
- Scoreboards didn't add properly
- Another addon is using player.json and is incompatible with lifesteal

Lifesteal working properly? Ignore this message with \xA7o/ls:joinerrorignore true\xA7r\xA7c or \xA7o?joinerrorignore true`);
        }
      }, 20);
...
Command Prefix: \xA7l${config.prefix}\xA7r | Get started by doing \xA7l${config.prefix}help\xA7r
...
// src/lifesteal/core_modules/reviveSystem.ts
var reviveSystem_exports = {};
__export(reviveSystem_exports, {
  handleRevive: () => handleRevive
});
import { GameMode as GameMode6, world as world21 } from "@minecraft/server";
function handleRevive(player) {
  if (!player.hasTag(`ls-revive`))
    return;
  if (!player.hasTag(`ls-dead`)) {
  ...
  if (player.getGameMode() === GameMode6.Spectator)
    player.setGameMode(GameMode6.Survival);
  player.removeTag(`ls-dead`);
  player.addEffect(`regeneration`, 20, { amplifier: 255, showParticles: false });
  player.addEffect(`resistance`, 60, { amplifier: 255, showParticles: false });
  const message = `\xA7bYou were revived${reviverName ? ` by \xA7f${reviverName}` : ``}\xA7b!`;
  player.onScreenDisplay.setActionBar(message);
  sendPlayerMessage(player, message);
  if (holBonus > 0)
    sendPlayerMessage(player, `\xA7bYou were given ${holBonus} extra heart${holBonus > 1 ? `s` : ``} because of your heart of life perk!`);
  if (player.getSpawnPoint()) {
    player.teleport(player.getSpawnPoint(), { dimension: player.getSpawnPoint().dimension });
  } else {
    const block = player.dimension.getTopmostBlock(world21.getDefaultSpawnLocation());
    player.teleport(block.above(), { dimension: world21.getDimension(`overworld`) });
  }
  return true;
}
var init_reviveSystem = __esm({
  "src/lifesteal/core_modules/reviveSystem.ts"() {
    init_databases();
    init_commonUtils();
  }
...
// src/lifesteal/runInterval.ts
var runInterval_exports = {};
import { system as system22, world as world22, GameMode as GameMode7, ItemStack as ItemStack6, InputPermissionCategory as InputPermissionCategory2 } from "@minecraft/server";
...
function handlePlayerHearts(player, score, health) {
  ...
  if (player.hasTag(`ls-dead`)) {
    player.setGameMode(GameMode7.Spectator);
    player.onScreenDisplay.setActionBar(`\xA7cYou lost all your hearts, you are \xA7ldead`);
...
function handleCompatibilityMode(player, score, health) {
...
function handleGhostBarrier(player) {
  const barrierDistance = +lifesteal.get(`ghostbarrier`);
  let spawnLocation = world22.getDefaultSpawnLocation();
  if (spawnLocation.y === 32767)
    spawnLocation.y = 90;
...
function exceededHearts(player) {
  ...
  for (let i = 0; i < inventorySize; i++) {
    ...
    if (!item || item.typeId !== `lifesteal:heart`)
      continue;
    inventory.container.setItem(i, new ItemStack6(item.typeId, item.amount));
  }
  const sendCommandFeedback = world22.gameRules.sendCommandFeedback;
  player.runCommand(`gamerule sendcommandfeedback false`);
  player.runCommand(`give @s lifesteal:heart ${amount}`);
  player.runCommand(`gamerule sendcommandfeedback ${sendCommandFeedback}`);
}
function setLoreItems(player) {
  const inventory = player.getComponent(`inventory`).container;
  if (lifesteal.get(`antilore`) === `true` || inventory.emptySlotsCount === inventory.size || system22.currentTick % 2 !== 0)
    return;
  for (let i = 0; i < inventory.size; i++) {
    const item = inventory.getSlot(i);
    if (!item.isValid || !item.hasItem())
      continue;
...
var init_runInterval = __esm({
  "src/lifesteal/runInterval.ts"() {
    ...
    system22.runInterval(() => {
      if (system22.currentTick % 200 === 0 && lifestealLogs.length > 500)
        lifestealLogs.length = 0;
      if (system22.currentTick % 12e3 === 0)
        addDefaultSettings();
      processSpectators();
      for (const player of world22.getAllPlayers()) {
        if (system22.currentTick % 1200 === 0)
          processDeadPlayer(player);
        handleRevive(player);
        ...
        if (player.hasTag(`heart-show_on`)) {
          player.onScreenDisplay.setActionBar(`\xA7a${(health.currentValue / 2).toFixed(1)}/${score}`);
        }
        if (player.hasTag(`heart-display_on`) && system22.currentTick % 20 === 0) {
          player.nameTag = `${player.name}
\xA7c${Math.round(health.currentValue / 2)}/${score}`;
        }
        handlePlayerHearts(player, score, health);
      }
    }, 1);
    lores = {
      "lifesteal:beacon_of_life": "\xA7r\xA77This is the beacon of life.\n\xA77Use this to revive a player.\n\n\xA77(( \xA7o\xA7fRight click/eat or do ?revive\xA77 to use ))",
      "lifesteal:heart_of_life": "\xA7r\xA77This is the heart of life.\n\xA77Permanently increase your revive heart count\n\n\xA77(( \xA7o\xA7fRight click/eat to use ))",
      "lifesteal:heart_fragment": "\xA7r\xA7dUse 4 of these to craft a Heart.",
      "lifesteal:heart": "\xA7r\xA77A heart is used to add a heart to your health.\n\n\xA77(( \xA7o\xA7fRight click/eat\xA77 to recieve a heart ))",
      "lifesteal:heart_essence": "\xA7r\xA7dUse this to craft a Heart."
    };
  }
});
...
// src/lifesteal/commands/heart.ts
init_commonUtils();
init_config();
init_handler();

// src/lifesteal/commands/heartSubcommands/heartShow.ts
init_commonUtils();
function heartShow(player) {
  const hadHeartTag = player.hasTag(`heart-show_on`);
  if (hadHeartTag)
    player.removeTag(`heart-show_on`);
  else
    player.addTag(`heart-show_on`);
  sendPlayerMessage(player, `\xA7aYour hearts are ${hadHeartTag ? `no longer` : `now`} displayed on your actionbar`);
}

// src/lifesteal/commands/heartSubcommands/heartDisplay.ts
init_commonUtils();
function heartDisplay(player) {
  const hadHeartTag = player.hasTag(`heart-display_on`);
  if (hadHeartTag)
    player.removeTag(`heart-display_on`);
  else
    player.addTag(`heart-display_on`);
  sendPlayerMessage(player, `\xA7aYour hearts are ${hadHeartTag ? `no longer` : `now`} displayed above your nametag`);
}

// src/lifesteal/commands/heartSubcommands/heartNotify.ts
init_commonUtils();
function heartNotify(player) {
  const hadHeartTag = player.hasTag(`heart-notify-off`);
  if (hadHeartTag)
    player.removeTag(`heart-notify-off`);
  else
    player.addTag(`heart-notify-off`);
  sendPlayerMessage(player, `\xA7aYou will ${hadHeartTag ? `now` : `no longer`} be notified when you gain/lose hearts`);
}

// src/lifesteal/commands/heartSubcommands/staffOnly/heartSet.ts
init_commonUtils();
init_databases();
init_ghostHandling();
import { GameMode } from "@minecraft/server";
function heartSet(player, args) {
  if (args.length > 3)
    return sendPlayerMessage(player, `\xA7cExtra arguments at the end of command`);
  const amount = args[2] ? +args[2] : +args[1];
  const target = args[2] || isNaN(amount) ? getPlayer(args[1]) : player;
  if (!target)
    return sendPlayerMessage(player, `\xA7cCouldn't find a player with that name ('${args[1] ?? ``}')`);
  if (isNaN(amount) || !Number.isInteger(amount))
    return sendPlayerMessage(player, `\xA7cExpected integer ('${amount}')`);
  const maxHearts = +lifesteal.get(`maxhearts`);
  if (!maxHearts) {
    console.warn(`Lifesteal settings - maxhearts is undefined`);
    return sendPlayerMessage(player, `\xA7cAn error occured while trying to run that command`);
  }
  if (amount > maxHearts || amount < 0)
    return sendPlayerMessage(player, `\xA7cPlease input a value from 0-${maxHearts}`);
  setScore(target, `ls-death`, amount);
  if (target.hasTag(`ls-dead`)) {
  ...
  sendPlayerMessage(player, `\xA7aSet ${target.name}'s hearts to ${amount} from ${targetsHeartCount}`);
}

// src/lifesteal/commands/heartSubcommands/staffOnly/settings/heartSetMax.ts
init_commonUtils();
init_databases();
function heartSetMax(player, args) {
  if (args.length > 2)
    return sendPlayerMessage(player, `\xA7cExtra arguments at the end of command`);
  const amount = +args[1];
  if (!args[1])
    return sendPlayerMessage(player, `\xA7aMax hearts is set to ${lifesteal.get(`maxhearts`)}`);
  if (isNaN(amount) || !Number.isInteger(amount))
    return sendPlayerMessage(player, `\xA7cExpected integer ('${args[1] ?? ``}')`);
  if (amount < 1 || amount > 200)
    return sendPlayerMessage(player, `\xA7cPlease input a number between 1-200`);
  lifesteal.set(`maxhearts`, `${amount}`);
  const heartItemCap2 = +(lifesteal.get(`heartitemcap`) ?? 0);
  if (heartItemCap2 > amount) {
    lifesteal.set(`heartitemcap`, amount);
    sendPlayerMessage(player, `\xA7aSet heart item cap to ${amount}`);
  }
  sendPlayerMessage(player, `\xA7aSet max hearts to ${amount}`);
}

// src/lifesteal/commands/heartSubcommands/staffOnly/settings/heartEnvsteal.ts
init_commonUtils();
init_databases();
function heartEnvsteal(player, args) {
  const newValue = args[1] ?? ``;
  if (!newValue)
    return sendPlayerMessage(player, `\xA7aEnvsteal set to ${lifesteal.get(`envsteal`)}`);
  if (![`true`, `false`].includes(newValue))
    return sendPlayerMessage(player, `\xA7cExpected boolean ('${newValue}')`);
  lifesteal.set(`envsteal`, newValue);
  sendPlayerMessage(player, `\xA7aSet environment steal to ${newValue}`);
}

// src/lifesteal/commands/heartSubcommands/staffOnly/settings/heartEnvdrop.ts
init_commonUtils();
init_databases();
function heartEnvdrop(player, args) {
  const newValue = args[1] ?? ``;
  if (!newValue)
    return sendPlayerMessage(player, `\xA7aEnvironment drop set to ${lifesteal.get(`envdrop`)}`);
  if (![`true`, `false`].includes(newValue))
    return sendPlayerMessage(player, `\xA7cExpected boolean ('${newValue}')`);
  lifesteal.set(`envdrop`, newValue);
  sendPlayerMessage(player, `\xA7aSet environment drop to ${newValue}`);
}

// src/lifesteal/commands/heartSubcommands/staffOnly/heartAdd.ts
init_commonUtils();
init_ghostHandling();
import { GameMode as GameMode2 } from "@minecraft/server";
function heartAdd(player, args) {
  if (args.length > 3)
    return sendPlayerMessage(player, `\xA7cExtra arguments at the end of command`);
  const amount = args[2] ? +args[2] : +args[1];
  const target = args[2] || isNaN(amount) ? getPlayer(args[1]) : player;
  ...
  if (target.hasTag(`ls-dead`)) {
    target.setGameMode(GameMode2.Survival);
    target.removeTag(`ls-dead`);
    target.onScreenDisplay.setActionBar(`\xA7bYou have been revived!`);
  }
  processAllDeadplayers();
  const newTargetsHeartCount = getScore(target, `ls-death`);
  sendPlayerMessage(player, `\xA7aAdded ${amount} hearts to ${target.name}'s heart count (from ${targetsHeartCount} to ${newTargetsHeartCount})`);
}

// src/lifesteal/commands/heartSubcommands/staffOnly/heartRemove.ts
init_commonUtils();
init_ghostHandling();
function heartRemove(player, args) {
  if (args.length > 3)
    return sendPlayerMessage(player, `\xA7cExtra arguments at the end of command`);
  const amount = args[2] ? +args[2] : +args[1];
  const target = args[2] || isNaN(amount) ? getPlayer(args[1]) : player;
  ...
  processAllDeadplayers();
  const newTargetsHeartCount = getScore(target, `ls-death`);
  sendPlayerMessage(player, `\xA7aRemoved ${amount} hearts from ${target.name}'s heart count (from ${targetsHeartCount} to ${newTargetsHeartCount})`);
}

// src/lifesteal/commands/heartSubcommands/staffOnly/heartGet.ts
init_commonUtils();
function heartGet(player, args) {
  if (args.length > 2)
    return sendPlayerMessage(player, `\xA7cExtra arguments at the end of command`);
  ...
  if (!target)
    return sendPlayerMessage(player, `\xA7cCouldn't find a player with that name ('${args[1] ?? ``}')`);
  const targetsHeartCount = getScore(target, `ls-death`);
  sendPlayerMessage(player, `\xA7a${target.name} currently has ${targetsHeartCount} hearts`);
}

// src/lifesteal/commands/heartSubcommands/staffOnly/settings/heartGain.ts
init_commonUtils();
init_databases();
function heartGain(player, args) {
  if (args.length > 2)
    return sendPlayerMessage(player, `\xA7cExtra arguments at the end of command`);
  const amount = +args[1];
  if (!args[1])
    return sendPlayerMessage(player, `\xA7aHearts gained on kill is set to ${lifesteal.get(`heartgain`)}`);
  if (isNaN(amount) || !Number.isInteger(amount))
    return sendPlayerMessage(player, `\xA7cExpected integer ('${args[1] ?? ``}')`);
  lifesteal.set(`heartgain`, `${amount}`);
  sendPlayerMessage(player, `\xA7aSet hearts gained on kill to ${amount}`);
}

// src/lifesteal/commands/heartSubcommands/staffOnly/settings/heartLose.ts
init_commonUtils();
init_databases();
function heartLose(player, args) {
  if (args.length > 2)
    return sendPlayerMessage(player, `\xA7cExtra arguments at the end of command`);
  const amount = +args[1];
  if (!args[1])
    return sendPlayerMessage(player, `\xA7aHearts lost on death is set to ${lifesteal.get(`heartlose`)}`);
  if (isNaN(amount) || !Number.isInteger(amount))
    return sendPlayerMessage(player, `\xA7cExpected integer ('${args[1] ?? ``}')`);
  lifesteal.set(`heartlose`, `${amount}`);
  sendPlayerMessage(player, `\xA7aSet hearts lost on death to ${amount}`);
}

// src/lifesteal/commands/heartSubcommands/staffOnly/settings/heartItemCap.ts
init_commonUtils();
init_databases();
function heartItemCap(player, args) {
  if (args.length > 2)
    return sendPlayerMessage(player, `\xA7cExtra arguments at the end of command`);
  const amount = +args[1];
  if (!args[1])
    return sendPlayerMessage(player, `\xA7aHeart item cap is set to ${lifesteal.get(`heartitemcap`)}`);
  if (isNaN(amount) || !Number.isInteger(amount))
    return sendPlayerMessage(player, `\xA7cExpected integer ('${args[1] ?? ``}')`);
  const maxHearts = +(lifesteal.get(`maxhearts`) ?? 0);
  if (amount > maxHearts || amount < 1)
    return sendPlayerMessage(player, `\xA7cPlease input a number from 1-${maxHearts}`);
  lifesteal.set(`heartitemcap`, `${amount}`);
  sendPlayerMessage(player, `\xA7aSet heart item cap to ${amount}`);
}

// src/lifesteal/commands/heartSubcommands/staffOnly/heartReset.ts
init_commonUtils();
init_databases();
init_ghostHandling();
import { GameMode as GameMode3 } from "@minecraft/server";
function heartReset(player, args) {
  if (args.length > 2)
    return sendPlayerMessage(player, `\xA7cExtra arguments at the end of command`);
  const target = args[1] ? getPlayer(args[1]) : player;
  if (!target)
  ...
  if (target.hasTag(`ls-dead`)) {
    ...
    target.removeTag(`ls-dead`);
    player.onScreenDisplay.setActionBar(`\xA7bYou have been revived!`);
  }
  processAllDeadplayers();
  sendPlayerMessage(player, `\xA7aReset ${target.name}'s heart count to ${startAmount}`);
}

// src/lifesteal/commands/heartSubcommands/heartCount.ts
init_commonUtils();
function heartCount(player) {
  sendPlayerMessage(player, `\xA7aYou have ${getScore(player, `ls-death`)} hearts`);
}

// src/lifesteal/commands/heartSubcommands/staffOnly/settings/heartMaxHolBonus.ts
init_commonUtils();
init_databases();
function heartMaxHolBonus(player, args) {
  if (args.length > 2)
    return sendPlayerMessage(player, `\xA7cExtra arguments at the end of command`);
  const amount = +args[1];
  if (!args[1])
    return sendPlayerMessage(player, `\xA7aMax Heart of Life bonus is set to ${lifesteal.get(`maxholbonus`)}`);
  if (isNaN(amount) || !Number.isInteger(amount))
    return sendPlayerMessage(player, `\xA7cExpected integer ('${args[1] ?? ``}')`);
  if (amount < 0)
    return sendPlayerMessage(player, `\xA7cPlease input a number more than 0`);
  lifesteal.set(`maxholbonus`, `${amount}`);
  sendPlayerMessage(player, `\xA7aSet Max Heart of Life bonus to ${amount}`);
}

// src/lifesteal/commands/heart.ts
new Command({
  name: `heart`,
  helpMenu: `\xA7e\xA7lLifesteal Help Page -- heart\xA7r\xA7g
> Heart stuff
...
> Examples: [
  ...
  altHelpMenu: `\xA7e\xA7lLifesteal Help Page -- heart\xA7r\xA7g
> Heart stuff
...
> Examples: [
  ...
  callback({ player, args, cmdData }) {
    if (!args[0]) {
      ...
      return sendPlayerMessage(player, cmdData.helpMenu.replaceAll(`\xA7e`, `\xA79`).replaceAll(`\xA7g`, `\xA7u`));
    }
    const foundStaffSubCommand = staffSubCommands[args[0].toLowerCase()];
    const foundSubCommand = subCommands[args[0].toLowerCase()];
    if ((player.commandPermissionLevel >= 2 || player.hasTag(`lifesteal-opped`)) && foundStaffSubCommand)
      foundStaffSubCommand(player, args);
    else if (foundSubCommand)
      foundSubCommand(player, args);
    else
      sendPlayerMessage(player, `\xA7cUnknown argument '${args[0]}'. Do ${config.prefix}help heart for more information on how to use this command`);
  }
});
var subCommands = {
  show: heartShow,
...
// src/lifesteal/commands/help.ts
init_commonUtils();
init_config();
init_handler();
import { CustomCommandStatus, system as system4 } from "@minecraft/server";
SlashCommandBuilder.createCommand({
  commandInfo: {
    name: `ls:help`,
    permissionLevel: 0,
    cheatsRequired: false,
    description: `Shows help page for lifesteal console commands`
  },
  callback(origin) {
    const player = origin.sourceEntity;
    if (player?.typeId !== `minecraft:player`)
      return { status: CustomCommandStatus.Failure, message: `Not a player` };
    system4.run(() => showHelp(player));
...
function showHelp(player) {
  sendPlayerMessage(player, `\xA7c----------------------------
\xA7c\xA7lLifesteal Help Page\xA7r
\xA7f\xA7lCommand Prefix: \xA7r\xA7c${config.prefix}
\xA7f\xA7lDo \xA7r\xA7c/ls:lifestealconsole "<command>"\xA7f\xA7l to run a command. Prefix optional

\xA7f\xA7le.g
\xA7r\xA7c/ls:lifestealconsole "withdraw 3"
/ls:lifestealconsole "revive ${player.name}"
\xA7r\xA7c----------------------------

\xA76Normal Commands --

\xA7e- \xA7lwithdraw <amount> \xA7r\xA7g| Withdraw one or more hearts from your heart count
\xA7e- \xA7lrevive <player> \xA7r\xA7g| Revive a player (Must have a heart of life in hand) 
\xA7e- \xA7ldeadplayers \xA7r\xA7g| Show all dead player's names
\xA7e- \xA7lheart <display/show> \xA7r\xA7g| Heart commands
\xA7e- \xA7lwelcome \xA7r\xA7g| Show the welcome GUI
\xA7e- \xA7lversion \xA7r\xA7g| Shows the current lifesteal version

\xA7r\xA72Ghost Commands --

\xA7a- \xA7lspectate <player> \xA7r\xA7q| Spectate a player without their coordinates being shown, sneak to stop spectating
\xA7a- \xA7ljumpscare <player> \xA7r\xA7q| Jumpscare a player (must be near them)
\xA7a- \xA7lghostpanel \xA7r\xA7q| Opens a panel where you can select a player to jumpscare/spectate

\xA7r\xA7sStaff Commands --\xA7r
${player.hasTag(config.staffTag) || player.commandPermissionLevel >= 2 ? `
\xA79- \xA7ladminpanel \xA7r\xA7u| Open the admin panel (Has a better way of setting settings)

\xA79- \xA7lheart <set/add/remove/max/...> \xA7r\xA7u| Heart commands such as set (Sets a max player's hearts), envsteal (Sets whether dying from mobs, fall damage, etc makes you lose a heart), etc
\xA79- \xA7llogs \xA7r\xA7u| Shows everytime a player used a command or gained a heart
\xA79- \xA7lget <item> <amount> \xA7r\xA7u| Get a lifesteal item without the need of an identifier
\xA79- \xA7lghostbarrier <amount> \xA7r\xA7u| Set the ghost barrier
\xA79- \xA7lforcerevive <player> \xA7r\xA7u| Revives a player even if you don't have a Heart of Life
\xA79- \xA7lforcekill <player> \xA7r\xA7u| Sets a player to 0 hearts and makes them a ghost
\xA79- \xA7lreviveall \xA7r\xA7u| Revive all players
\xA79- \xA7lreviveamount <amount> \xA7r\xA7u| Set the amount of hearts a player is given when they are revived
\xA79- \xA7lbanghosts <true/false> \xA7r\xA7u| Whether people get banned when they reach 0 hearts or not
\xA79- \xA7lstartamount <amount> \xA7r\xA7u| Set the amount of hearts a new player starts with
\xA79- \xA7lcompatibility <arg> <true/false> \xA7r\xA7u| Settings for compatibility with other addons

\xA7r\xA7f\xA7lDo ${config.prefix}help <commandName> for information on that command!` : `\xA7cYou do not have permission to view staff commands! Do \xA7l/tag @s add ${config.staffTag}\xA7r\xA7c to get permission.

\xA7r\xA7f\xA7lDo ${config.prefix}help <commandName> for information on that command!`}`);
}

// src/lifesteal/commands/revive.ts
init_databases();
init_commonUtils();
init_config();
init_handler();
init_ghostHandling();
import { CustomCommandParamType as CustomCommandParamType2, CustomCommandStatus as CustomCommandStatus2, system as system5 } from "@minecraft/server";
SlashCommandBuilder.createCommand({
  commandInfo: {
    name: `ls:revive`,
    ...
    mandatoryParameters: [{ name: `targetName`, type: CustomCommandParamType2.String }],
    cheatsRequired: false
  },
  callback(origin, targetName) {
    const player = origin.sourceEntity;
    if (player?.typeId !== `minecraft:player`)
...
new Command({
  name: `revive`,
  helpMenu: `\xA7e\xA7lLifesteal Help Page -- revive\xA7r\xA7g
...
function reviveCommand(player, targetName) {
  const inventory = player.getComponent(`inventory`);
  const item = inventory.container.getItem(player.selectedSlotIndex);
  if (!item || item.typeId !== `lifesteal:beacon_of_life`)
    return sendPlayerMessage(player, `\xA7cYou must have a beacon of life in your hand to revive a player!`);
  const target = deadplayers.get(targetName);
  if (!target) {
    return sendPlayerMessage(player, `\xA7cFailed to find player. Make sure the capitalization is correct and they are an eliminated player. Do \xA7o/ls:deadplayers\xA7r\xA7c or \xA7o?deadplayers\xA7r\xA7c for a list of eliminated players`);
  }
  revivelist.set(`${target.toLowerCase()}`, `${target.toLowerCase()}.,${player.name}`);
  deadplayers.delete(target);
  if (player.runCommand(`clear @s lifesteal:beacon_of_life 0 1`).successCount <= 0)
    return;
  sendAllMessage2(`\xA7b${player.name} has revived ${target}`);
...
// src/lifesteal/commands/version.ts
init_changelog();
init_commonUtils();
init_handler();
new Command({
...
// src/lifesteal/commands/misc/deadplayers.ts
init_databases();
init_commonUtils();
init_config();
init_handler();
import { CustomCommandStatus as CustomCommandStatus4, system as system7 } from "@minecraft/server";
SlashCommandBuilder.createCommand({
  commandInfo: {
    name: `ls:deadplayers`,
    description: `Get a list of all eliminated players`,
    permissionLevel: 0,
    cheatsRequired: false
  },
  callback(origin) {
    ...
    system7.run(() => {
      const deadPlayersList = Array.from(deadplayers.keys());
      sendPlayerMessage(player, `\xA7e\xA7lDead Players ---\xA7r\xA7g
${deadPlayersList.length !== 0 ? deadPlayersList.join(`
`) : `\xA76No dead players found!`}`);
    });
...
new Command({
  name: `deadplayers`,
  helpMenu: `\xA7e\xA7lLifesteal Help Page -- deadplayers\xA7r\xA7g
> Get a list of all eliminated players

> Usage: ${config.prefix}deadplayers

> Examples: [
    ${config.prefix}deadplayers
]`,
  callback({ player }) {
    const deadPlayersList = Array.from(deadplayers.keys());
    sendPlayerMessage(player, `\xA7e\xA7lDead Players ---\xA7r\xA7g
...
// src/lifesteal/commands/misc/welcome.ts
init_commonUtils();
init_config();
init_handler();
init_welcome();
import { CustomCommandStatus as CustomCommandStatus5, system as system8 } from "@minecraft/server";
SlashCommandBuilder.createCommand({
  commandInfo: {
    name: `ls:welcome`,
    description: `Opens the welcome menu showed when joining`,
    permissionLevel: 0,
    cheatsRequired: false
  },
  callback(origin) {
    ...
    system8.run(() => openWelcomeGui(player));
    return { status: CustomCommandStatus5.Success };
  }
});
new Command({
  name: `welcome`,
  helpMenu: `\xA7e\xA7lLifesteal Help Page -- welcome\xA7r\xA7g
...
> Examples: [
  ...
  callback({ player, args }) {
    if (!player.hasTag(`welcome-gui_done`))
      return sendPlayerMessage(player, `\xA73Close chat to open GUI`);
    player.removeTag(`welcome-gui_done`);
    sendPlayerMessage(player, `\xA73Close chat to open GUI`);
    if (!player.hasTag("welcome-gui_done")) {
      player.addTag("welcome-gui_done");
      openWelcomeGui(player);
    }
...
// src/lifesteal/commands/ghostsOnly/jumpscare.ts
init_commonUtils();
init_handler();
import { system as system9 } from "@minecraft/server";
function jumpscarePlayer(target, player, ignoreCooldown = false) {
  if (!target || target?.hasTag(`ls-dead`) && target.id !== player.id) {
    player.sendMessage(`\xA7cFailed to find player.`);
    player.playSound(`random.break`, { volume: 0.5, pitch: 8 });
    return;
  }
  const cooldown = player.getDynamicProperty(`jumpscare-cooldown`);
  if ((cooldown ? cooldown > Date.now() : false) && target.id !== player.id && !ignoreCooldown) {
    sendPlayerMessage(player, `\xA7cYou are on cooldown! \xA76${timeStamp(cooldown - Date.now())}\xA7c left.`);
    player.playSound(`random.break`, { volume: 0.5, pitch: 8 });
    return;
  }
  if (!target) {
    sendPlayerMessage(player, `\xA7cFailed to find player.`);
    player.playSound(`random.break`, { volume: 0.5, pitch: 8 });
    return;
  }
  sendPlayerMessage(player, `\xA7cJumpscared \xA76${target.name}`);
  target.addEffect(`darkness`, 60);
  target.playSound(`mob.warden.nearby_closest`, { volume: 0.2 });
  system9.runTimeout(() => {
    target.playSound(`mob.enderdragon.growl`, { volume: 0.5 });
    target.playSound(`mob.enderdragon.death`, { volume: 0.5 });
    target.playSound(`mob.enderdragon.growl`, { pitch: 1.3, volume: 1 });
    target.playSound(`mob.enderdragon.death`, { pitch: 1.3, volume: 0.5 });
    target.playSound(`mob.warden.emerge`, { pitch: 0.5, volume: 0.3 });
    target.playSound(`mob.warden.emerge`, { pitch: 0.5, volume: 0.4 });
    target.playSound(`mob.warden.emerge`, { pitch: 0.5, volume: 0.5 });
    const randomnumber = randomN(1, 3);
    if (randomnumber === 1)
      target.onScreenDisplay.setTitle(`\xA7c\xA7lboo`);
    if (randomnumber === 2)
      target.onScreenDisplay.setTitle(`\xA7c\xA7lrahh!!`);
    if (randomnumber === 3)
      target.onScreenDisplay.setTitle(`\xA7c\xA7lbig roar`);
    target.onScreenDisplay.updateSubtitle(`\xA7c\xA7ljumpscared by ${player.name}`);
  }, 40);
  if (target.id !== player.id)
    player.setDynamicProperty(`jumpscare-cooldown`, Date.now() + 6e4);
}
new Command({
  name: `jumpscare`,
  callback({ player, args }) {
...
// src/lifesteal/commands/ghostsOnly/ghostpanel.ts
init_commonUtils();
init_handler();

// src/lifesteal/gui/ghostpanel.ts
init_databases();
init_spectate();
init_commonUtils();
import { system as system11, world as world7 } from "@minecraft/server";
import { ActionFormData as ActionFormData2, FormCancelationReason as FormCancelationReason2, ModalFormData, uiManager } from "@minecraft/server-ui";
function ghostPanel(player) {
  const spectateOn = lifesteal.get(`spectating`);
  const gpMain = new ActionFormData2().title(`Ghost GUI`).button(spectateOn === `true` ? `Spectate player` : `Spectate player
\xA7cDisabled`, `textures/ui/FriendsIcon.png`).button(`Jumpscare player`, `textures/blocks/pumpkin_face_off.png`).button(`Close`, `textures/ui/cancel.png`);
  gpMain.show(player).then((result) => {
    uiManager.closeAllForms(player);
    if (result.canceled && result.cancelationReason === FormCancelationReason2.UserBusy)
      return ghostPanel(player);
    ...
    switch (result.selection) {
      default:
        ...
        break;
      case 0:
        let spectateGUI = function() {
          const gui = new ModalFormData().title(`Spectate player`).dropdown(`Players`, [...players.map((x) => x.name), `Unselected`], { defaultValueIndex: players.length }).submitButton(`Spectate`);
          gui.show(player).then(async (result2) => {
            player.playSound(`note.pling`, { pitch: 2, volume: 0.1 });
            if (result2.canceled || result2.formValues[0] === players.length)
              return;
            const formValues = result2.formValues;
            const target = players[formValues[0]];
            if (!target || target?.hasTag(`ls-dead`)) {
              if (target === player)
                return sendPlayerMessage(player, `\xA7cYou cannot spectate yourself!`);
              player.sendMessage(`\xA7cFailed to find player.`);
              player.playSound(`random.break`, { volume: 0.5, pitch: 8 });
              return;
            }
            if (ghostSpectateMap.has(player.name))
              return sendPlayerMessage(player, `\xA7cAlready spectating a player! Sneak to cancel`);
            player.camera.fade({ fadeColor: { red: 0, blue: 0, green: 0 }, fadeTime: { fadeInTime: 0.5, fadeOutTime: 0.5, holdTime: 0.5 } });
            await system11.waitTicks(9);
            ghostSpectateMap.set(player.name, target.name);
            sendPlayerMessage(target, `\xA7cGhost ${player.name} is watching you.`);
            sendPlayerMessage(player, `\xA77[Sneak] to stop spectating`);
          });
        };
        if (spectateOn !== `true`)
          return ghostPanel(player);
        ...
        break;
      case 1:
        let jumpscareGUI = function() {
          const gui = new ModalFormData().title(`Jumpscare player`).dropdown(`Players`, [...players.map((x) => x.name), `Unselected`], { defaultValueIndex: players.length }).submitButton(`Jumpscare`);
          gui.show(player).then((result2) => {
            player.playSound(`note.pling`, { pitch: 2, volume: 0.1 });
            if (result2.canceled || result2.formValues[0] === players.length)
              return;
            const formValues = result2.formValues;
...
new Command({
  name: `ghostpanel`,
  aliases: [`gp`, `ghostgui`, `gg`],
  helpMenu: `\xA7e\xA7lLifesteal Help Page -- ghostpanel\xA7r\xA7g
> Show the welcome GUI

> Usage: ${config.prefix}ghostpanel
...
// src/lifesteal/commands/staffOnly/adminpanel.ts
init_commonUtils();
init_config();
init_handler();
init_adminpanel();
import { CommandPermissionLevel as CommandPermissionLevel2, CustomCommandStatus as CustomCommandStatus6, system as system13 } from "@minecraft/server";
SlashCommandBuilder.createCommand({
  commandInfo: {
    name: `ls:adminpanel`,
  ...
  callback(origin, ...args) {
    ...
    system13.run(() => adminPanel(player));
...
new Command({
  ...
  staffOnly: true,
  helpMenu: `\xA7e\xA7lLifesteal Help Page -- adminpanel\xA7r\xA7g
> Show the welcome GUI

> Usage: ${config.prefix}adminpanel
...
// src/lifesteal/commands/staffOnly/logs.ts
init_databases();
init_commonUtils();
init_config();
init_handler();
new Command({
  name: `logs`,
  staffOnly: true,
  helpMenu: `\xA7e\xA7lLifesteal Help Page -- logs\xA7r\xA7g
> See lifesteal logs (When a player gains or loses a heart, when a player uses a command, etc)
...
> Examples: [
  ...
  callback({ player, args }) {
    lifestealLogs.filter((v, i) => lifestealLogs.indexOf(v) !== i);
    sendPlayerMessage(player, `\xA7aLifesteal Logs:
(The time before \xA7q>\xA7a is in the UTC time zone)
${lifestealLogs.join(`
`)}
-----
\xA7aNote: Logs reset when the world restarts or when it hits 500`);
  }
});

// src/lifesteal/commands/staffOnly/op.ts
init_commonUtils();
init_config();
init_handler();
new Command({
  name: `op`,
  staffOnly: true,
  aliases: [`makeop`, `operator`],
  callback({ player, args }) {
    if (!args[0]) {
      if (player.hasTag(config.staffTag)) {
        player.removeTag(config.staffTag);
        sendPlayerMessage(player, `\xA7a${player.name} is no longer lifesteal-opped!`);
        return;
      }
      sendPlayerMessage(player, `\xA7a${player.name} is now lifesteal-opped!`);
      player.addTag(config.staffTag);
      return;
    }
    let msg = !args[1] ? args[0] : args.join(` `).split(/\"/g)[1];
    let target = getPlayer(msg);
    if (!target)
      return sendPlayerMessage(player, `\xA7cPlayer not found (${msg})`);
    if (target.hasTag(config.staffTag)) {
      sendPlayerMessage(player, `\xA7a${target.name} is no longer lifesteal-opped!`);
      target.removeTag(config.staffTag);
      return;
    }
    target.addTag(config.staffTag);
    sendPlayerMessage(player, `\xA7a${target.name} is now lifesteal-opped!`);
  }
});

// src/lifesteal/commands/staffOnly/settings/banghosts.ts
init_databases();
init_commonUtils();
init_handler();
init_ghostHandling();
new Command({
  ...
  callback({ player, args }) {
    if (!args[0])
      return sendPlayerMessage(player, `\xA7aBanghosts is set to \xA7f${lifesteal.get(`banghosts`)}`);
    if (![`true`, `false`].includes(args[0].toLowerCase()))
      return sendPlayerMessage(player, `\xA7cMust be true or false`);
    lifesteal.set(`banghosts`, args[0].toLowerCase());
    processAllDeadplayers();
    sendPlayerMessage(player, `\xA7aSet ban ghosts to \xA7f${args[0]}`);
  }
});

// src/lifesteal/commands/staffOnly/settings/compatibility.ts
init_databases();
init_commonUtils();
init_config();
init_handler();
new Command({
  name: `compatibility`,
  staffOnly: true,
  helpMenu: `\xA7e\xA7lLifesteal Help Page -- compatibility\xA7r\xA7g
> Set addon compatibility with other mods, by enabling these options, they will remove certain lifesteal features but enable for lifesteal to be used with other mods

> Usage: ${config.prefix}compatibility <option> <true/false>

> Args: [
    useplayerjson - Whether to use player.json, turning this off will enable a backup heart system which is scuffed but works
    antilore - Turns item lore off when enabled, allowing for compatibility with anti-cheats with a lore check
]

> Examples: [
    ${config.prefix}compatibility
    ${config.prefix}compatibility useplayerjson
    ${config.prefix}compatibility antilore true
]`,
  callback({ player, args, cmdData }) {
    if (!args[0]) {
      return sendPlayerMessage(player, cmdData.helpMenu);
    ...
    switch (args.shift()) {
      case `antilore`:
        if (!args[0])
          return sendPlayerMessage(player, `\xA7aantiLore is set to \xA7f${lifesteal.get(`antilore`)}`);
        if (![`true`, `false`].includes(args[0].toLowerCase()))
          return sendPlayerMessage(player, `\xA7cMust be true or false`);
        lifesteal.set(`antilore`, args[0].toLowerCase());
        sendPlayerMessage(player, `\xA7aSet antiLore to \xA7f${args[0]}`);
        break;
      case `useplayerjson`:
        if (!args[0])
          return sendPlayerMessage(player, `\xA7ausePlayerJson is set to \xA7f${lifesteal.get(`useplayerjson`)}`);
        if (![`true`, `false`].includes(args[0].toLowerCase()))
          return sendPlayerMessage(player, `\xA7cMust be true or false`);
        lifesteal.set(`useplayerjson`, args[0].toLowerCase());
        sendPlayerMessage(player, `\xA7aSet usePlayerJson to \xA7f${args[0]}`);
        break;
    }
  }
});

// src/lifesteal/commands/staffOnly/settings/ghostbarrier.ts
init_databases();
init_commonUtils();
init_config();
init_handler();
new Command({
  name: `ghostbarrier`,
  staffOnly: true,
  helpMenu: `\xA7e\xA7lLifesteal Help Page -- ghostbarrier\xA7r\xA7g
> Set the barrier for ghosts (Ghosts can't go beyond the barrier, default is 100)

> Usage: ${config.prefix}ghostbarrier <amount>

> Examples: [
    ${config.prefix}ghostbarrier ${randomN(4, 6)}
${config.prefix}ghostbarrier ${randomN(5, 7)}
]`,
  callback({ player, args }) {
    const amount = +args[0];
    if (!args[0])
      return sendPlayerMessage(player, `\xA7aThe current radius of the ghost barrier is \xA7f${lifesteal.get(`ghostbarrier`)}`);
    if (args[0] == `disable`) {
      lifesteal.set(`ghostbarrier`, `disabled`);
      sendPlayerMessage(player, `\xA7aDisabled the \xA7fGhost Barrier`);
      return;
    }
    if (isNaN(amount))
      return sendPlayerMessage(player, `\xA7cThat isn't a number! To disable the ghost barrier, do \xA7l?ghostbarrier disable`);
    if (amount < 5)
      return sendPlayerMessage(player, `\xA7cGhost barrier can't be less than 5!`);
    lifesteal.set(`ghostbarrier`, `${amount}`);
    sendPlayerMessage(player, `\xA7aSet the \xA7fGhost Barrier\xA7a to \xA7f${amount}`);
  }
});

// src/lifesteal/commands/staffOnly/settings/reviveamount.ts
init_databases();
init_commonUtils();
init_handler();
new Command({
  ...
  callback({ player, args }) {
    ...
    if (isNaN(amount) || amount < 1 || amount > 15)
      return sendPlayerMessage(player, `\xA7cYou can only set the revive amount to numbers from 1-15!`);
    if (+lifesteal.get(`heartlose`) > amount)
      return sendPlayerMessage(player, `\xA7cYou can't set the revive amount lower than the hearts lost amount!`);
    lifesteal.set(`reviveamount`, `${amount}`);
    sendPlayerMessage(player, `\xA7aSet the revive amount to ${amount}`);
  }
});

// src/lifesteal/commands/staffOnly/settings/startamount.ts
init_databases();
init_commonUtils();
init_handler();
new Command({
  ...
  callback({ player, args }) {
    ...
    if (isNaN(amount) || amount < 5 || amount > 15)
      return sendPlayerMessage(player, `\xA7cYou can only set the start amount to numbers from 5-15!`);
    lifesteal.set(`startamount`, `${amount}`);
    sendPlayerMessage(player, `\xA7aSet the start amount to ${amount}`);
  }
});

// src/lifesteal/commands/staffOnly/settings/changePrefix.ts
init_handler();
init_config();
import { CustomCommandParamType as CustomCommandParamType4, CustomCommandStatus as CustomCommandStatus7, Player as Player22, world as world13 } from "@minecraft/server";
SlashCommandBuilder.createEnum(`ls:newPrefix`, [
  `!`,
  `@`,
  `#`,
  `$`,
...
SlashCommandBuilder.createCommand({
  commandInfo: {
    name: `ls:prefix`,
    description: `Get or change the prefix that lifesteal commands use. This doesn't affect slash commands`,
    permissionLevel: 1,
    optionalParameters: [{ type: CustomCommandParamType4.Enum, name: `ls:newPrefix` }],
    cheatsRequired: false
  },
  callback(origin, newPrefix) {
    const player = origin.sourceEntity;
    if (!(player instanceof Player22))
      return { status: CustomCommandStatus7.Failure, message: `Not a player` };
    if (!newPrefix)
      return { status: CustomCommandStatus7.Success, message: `Current prefix: ${config.prefix}` };
    world13.setDynamicProperty(`lifesteal:prefix`, newPrefix === `?` ? void 0 : newPrefix);
    config.prefix = newPrefix;
    return { status: CustomCommandStatus7.Success, message: `Changed prefix to ${newPrefix}` };
  }
});

// src/lifesteal/commands/staffOnly/utility/forcerevive.ts
init_databases();
init_commonUtils();
init_config();
init_handler();
...
new Command({
  name: `forcerevive`,
  staffOnly: true,
  helpMenu: `\xA7e\xA7lLifesteal Help Page -- forcerevive\xA7r\xA7g
> Revive a dead player with no heart of life

> Usage: ${config.prefix}forcerevive <player>

> Examples: [
    ${config.prefix}forcerevive GBShadow550
]`,
  callback({ player, args, cmdData }) {
    if (!args[0])
      return sendPlayerMessage(player, cmdData.helpMenu);
    const target = deadplayers.get(args[0]);
    if (!target) {
      for (const players of world14.getAllPlayers()) {
        if (players.name.toLowerCase() == args.join(` `).toLowerCase())
          return sendPlayerMessage(player, `\xA7cThat player is not a ghost!`);
      }
      return sendPlayerMessage(player, `\xA7cFailed to find player. Make sure the capitalization is correct and they are a ghost`);
    }
    revivelist.set(`${target.toLowerCase()}`, `${target.toLowerCase()}`);
    deadplayers.delete(target);
    sendPlayerMessage(player, `\xA7aRevived \xA7f${target}`);
    processAllDeadplayers();
  }
});

// src/lifesteal/commands/staffOnly/utility/get.ts
init_commonUtils();
init_config();
init_handler();
import { ItemStack as ItemStack3 } from "@minecraft/server";
new Command({
  name: `get`,
  aliases: [`give`],
  staffOnly: true,
  helpMenu: `\xA7e\xA7lLifesteal Help Page -- get\xA7r\xA7g
> See a player's heart count

> Usage: ${config.prefix}get <player>

> Examples: [
    ${config.prefix}get heart ${randomN(2, 4)}
    ${config.prefix}get heart_of_life ${randomN(3, 5)}
]`,
  callback({ player, args, cmdData }) {
    const acceptable = [
      `heart`,
    ...
    if (!args[0])
      return sendPlayerMessage(player, cmdData.helpMenu);
    if (!acceptable.includes(args[0].toLowerCase()))
      return sendPlayerMessage(player, `\xA7cThat isn't an item! Do ?help get for a list of items`);
    if (!args[1])
      args[1] = `1`;
    const amount = +args[1];
    if (isNaN(amount))
    ...
    switch (args[0]) {
      case `beacon_of_life`:
        try {
          ...
          let inventorySize = inventory.container.size;
          for (let i = 0; i < inventorySize; i++) {
            let item = inventory.container.getItem(i);
            if (!item)
              continue;
            let newItem = new ItemStack3(`lifesteal:beacon_of_life`, item.amount);
            if (item.typeId == `lifesteal:heart`) {
              inventory.container.setItem(i, newItem);
            }
          }
          player.runCommand(`give @s lifesteal:beacon_of_life ${amount}`);
        } catch {
          sendPlayerMessage(player, `\xA7cFailed to give item`);
        }
        break;
      case `heart`:
        try {
          ...
          for (let i = 0; i < inventorySize; i++) {
            ...
            if (!item)
              continue;
            let newItem = new ItemStack3(`lifesteal:heart`, item.amount);
            if (item.typeId == `lifesteal:heart`) {
              inventory.container.setItem(i, newItem);
            }
          }
      ...
      case `heart_fragment`:
        try {
          ...
          for (let i = 0; i < inventorySize; i++) {
            ...
            if (!item)
              continue;
            let newItem = new ItemStack3(`lifesteal:heart_fragment`, item.amount);
            if (item.typeId == `lifesteal:heart_fragment`) {
              inventory.container.setItem(i, newItem);
            }
          }
          player.runCommand(`give @s lifesteal:heart_fragment ${amount}`);
        } catch {
          sendPlayerMessage(player, `\xA7cFailed to give item`);
        }
        break;
      case `heart_essence`:
        try {
          ...
          for (let i = 0; i < inventorySize; i++) {
            ...
            if (!item)
              continue;
            let newItem = new ItemStack3(`lifesteal:heart_essence`, item.amount);
            if (item.typeId == `lifesteal:heart_essence`) {
              inventory.container.setItem(i, newItem);
            }
          }
          player.runCommand(`give @s lifesteal:heart_essence ${amount}`);
        } catch {
          sendPlayerMessage(player, `\xA7cFailed to give item`);
        }
        break;
      case `heart_of_life`:
        try {
          ...
          for (let i = 0; i < inventorySize; i++) {
            ...
            if (!item)
              continue;
            let newItem = new ItemStack3(`lifesteal:heart_of_life`, item.amount);
            if (item.typeId == `lifesteal:heart_of_life`) {
              inventory.container.setItem(i, newItem);
            }
          }
          player.runCommand(`give @s lifesteal:heart_of_life ${amount}`);
        } catch {
          sendPlayerMessage(player, `\xA7cFailed to give item`);
        }
        break;
      case `withdraw_heart`:
        try {
          ...
          for (let i = 0; i < inventorySize; i++) {
            ...
            if (!item)
              continue;
            let newItem = new ItemStack3(`lifesteal:withdraw_heart`, item.amount);
            if (item.typeId == `lifesteal:withdraw_heart`) {
              inventory.container.setItem(i, newItem);
            }
          }
          player.runCommand(`give @s lifesteal:withdraw_heart ${amount}`);
        } catch {
          sendPlayerMessage(player, `\xA7cFailed to give item`);
        }
        break;
...
// src/lifesteal/commands/staffOnly/utility/kill.ts
init_commonUtils();
init_config();
init_handler();
init_ghostHandling();
new Command({
  name: `forcekill`,
  staffOnly: true,
  helpMenu: `\xA7e\xA7lLifesteal Help Page -- forcekill\xA7r\xA7g
> Kill a player and set them to 0 hearts

> Usage: ${config.prefix}forcekill <player>

> Examples: [
    ${config.prefix}forcekill GBShadow550
]`,
  callback({ player, args, cmdData }) {
    if (!args[0])
      return sendPlayerMessage(player, cmdData.helpMenu);
    let target = getPlayer(args.join(` `));
    if (!target)
      return sendPlayerMessage(player, `\xA7cFailed to find player, player must be online`);
    setScore(target, `ls-death`, 0);
    processAllDeadplayers();
    sendPlayerMessage(player, `\xA7aKilled \xA7f${target.name}`);
  }
});

// src/lifesteal/commands/staffOnly/utility/reviveall.ts
init_databases();
init_commonUtils();
init_config();
init_handler();
init_ghostHandling();
import { ButtonState as ButtonState2, InputButton as InputButton2, system as system14 } from "@minecraft/server";
new Command({
  name: `reviveall`,
  staffOnly: true,
  helpMenu: `\xA7e\xA7lLifesteal Help Page -- reviveall\xA7r\xA7g
...
> Examples: [
  ...
  callback({ player, args }) {
    sendPlayerMessage(player, `\xA7aAre you sure you want to revive all dead players? \xA7fSneak twice to confirm.`);
    if (![`true`, `false`].includes(args[0]))
      args[0] = `false`;
    handlePlayerSneak(player, JSON.parse(args[0]));
  }
});
async function handlePlayerSneak(player, hideRevive = false) {
  const tickStarted = system14.currentTick;
  const timeWindow = 200;
  const shiftTimeWindow = 20;
  let shiftTimeLeft = shiftTimeWindow;
  let lastSneakState;
  let validTimesPressed = 0;
  while (system14.currentTick < tickStarted + timeWindow && validTimesPressed !== 2) {
    const sneakState = player.inputInfo.getButtonState(InputButton2.Sneak);
    if (sneakState === ButtonState2.Pressed && lastSneakState === ButtonState2.Released) {
      validTimesPressed++;
  ...
  if (validTimesPressed !== 2)
    return sendPlayerMessage(player, `\xA7aCanceled revive all`);
  sendPlayerMessage(player, `\xA7aPlease wait..`);
  const reviver = hideRevive ? `` : `.,${player.name}`;
  for (const playerName of deadplayers.keys()) {
    revivelist.set(`${playerName.toLowerCase()}`, `${playerName.toLowerCase()}${reviver}`);
    deadplayers.delete(playerName);
  }
  sendPlayerMessage(player, `\xA7aRevived all dead players!`);
  processAllDeadplayers();
}

// src/lifesteal/commands/staffOnly/debug/fakeplayer.ts
init_commonUtils();
init_handler();
init_databases();
new Command({
  ...
  callback({ player, args }) {
    if (!args[0]) {
      const randomName = random(10, true);
      deadplayers.set(randomName, randomName);
      sendPlayerMessage(player, randomName);
      return;
    }
    deadplayers.set(`${args[0]}`, `${args[0]}`);
  }
});

// src/lifesteal/commands/staffOnly/debug/openReviveGui.ts
init_handler();
init_reviveGui();
new Command({
  name: `openrevivegui`,
  ...
  callback({ player, args }) {
    if (![`true`, `false`].includes(args[0]))
      args[0] = `false`;
    reviveGuiForm(player, false, JSON.parse(args[0]));
  }
});

// src/lifesteal/commands/staffOnly/debug/settings.ts
init_commonUtils();
init_handler();
init_databases();
new Command({
  ...
  staffOnly: true,
  callback({ player }) {
    for (const e of lifesteal.entries()) {
      sendPlayerMessage(player, `${e[0]} - ${e[1]}`);
    }
  }
});

// src/lifesteal/commands/staffOnly/debug/revivelist.ts
init_commonUtils();
init_handler();
init_ghostHandling();
init_databases();
new Command({
  ...
  callback({ player, args }) {
    switch (args[0]) {
      case `clear`:
        return revivelist.clear();
      case `add`: {
        if (!args[1])
          return console.warn(`no name`);
        const reviver = args.includes(`--h`) ? `` : `.,${player.name}`;
        revivelist.set(args[1].toLowerCase(), `${args[1].toLowerCase()}${reviver}`);
        if (args.includes(`--p`))
          processAllDeadplayers();
        return;
      }
    ...
    let currentColor = 9;
    let currentNoReviveColor = `f`;
    sendPlayerMessage(player, `${Array.from(revivelist.values()).map((x) => x.includes(`.,`) ? `\xA7${currentColor === 9 ? currentColor = 2 : currentColor = 9}${x}` : `\xA7${currentNoReviveColor === `f` ? currentNoReviveColor = `7` : currentNoReviveColor = `f`}${x}`)}`);
  }
});

// src/lifesteal/commands/staffOnly/debug/deadplayersRaw.ts
init_commonUtils();
init_handler();
init_ghostHandling();
init_databases();
new Command({
  ...
  callback({ player, args }) {
    switch (args[0]) {
      case `clear`:
        return deadplayers.clear();
      case `add`: {
        if (!args[1])
          return console.warn(`no name`);
        deadplayers.set(args[1], args[1]);
        if (args.includes(`--p`))
          processAllDeadplayers();
        return;
      }
    }
    sendPlayerMessage(player, `${Array.from(deadplayers.entries())}`);
  }
});

// src/lifesteal/commands/staffOnly/debug/errorIgnore.ts
init_handler();
init_commonUtils();
import { CustomCommandParamType as CustomCommandParamType5, CustomCommandStatus as CustomCommandStatus8, system as system15, world as world16 } from "@minecraft/server";
SlashCommandBuilder.createCommand({
  commandInfo: {
    name: `ls:joinerrorignore`,
    description: `Ignore the error message sent by lifesteal when it detects it isn't working`,
    mandatoryParameters: [{ name: `shouldIgnore`, type: CustomCommandParamType5.Boolean }],
    permissionLevel: 1,
    cheatsRequired: false
  },
  callback(origin, shouldIgnore) {
    world16.setDynamicProperty(`lifesteal:ignore-join-error`, shouldIgnore);
    system15.run(() => {
      sendAllMessage2(`\xA7aError ignore set to ${shouldIgnore}`);
    });
    return { status: CustomCommandStatus8.Success };
  }
});
new Command({
  name: `joinerrorignore`,
  helpMenu: `why`,
  callback({ player, args }) {
    if (![`true`, `false`].includes(args[0]))
      return sendPlayerMessage(player, `\xA7cExpected boolean ('${args[0]}')`);
    const shouldIgnore = JSON.parse(args[0]);
    world16.setDynamicProperty(`lifesteal:ignore-join-error`, shouldIgnore);
    sendAllMessage2(`\xA7aError ignore set to ${args[0]}`);
  }
});

// src/lifesteal/commands/lifestealConsole.ts
init_handler();
import { CustomCommandParamType as CustomCommandParamType6, CustomCommandStatus as CustomCommandStatus9, system as system16 } from "@minecraft/server";
SlashCommandBuilder.createCommand({
  commandInfo: {
    name: `ls:console`,
    description: `Run legacy lifesteal commands (they're like old discord commands. do /ls:help for a list)`,
    permissionLevel: 0,
    mandatoryParameters: [{ name: `lifestealCommandLine`, type: CustomCommandParamType6.String }],
    cheatsRequired: false
  },
  callback(origin, lifestealCommandLine) {
    const player = origin.sourceEntity;
    if (player?.typeId !== `minecraft:player`)
      return { status: CustomCommandStatus9.Failure, message: `Not a player` };
    system16.run(() => {
      runLifestealCommand(player, lifestealCommandLine);
    });
    return { status: CustomCommandStatus9.Success };
  }
});

// src/lifesteal/events/system/beforeEvents/startUp.ts
init_reviveGui();
init_handler();
import { system as system17 } from "@minecraft/server";
system17.beforeEvents.startup.subscribe((data) => {
  SlashCommandBuilder.registerCommands(data.customCommandRegistry);
  data.blockComponentRegistry.registerCustomComponent(`lifesteal:bol`, {
    onPlayerInteract: (data2) => {
      const player = data2.player;
      if (data2.block.typeId === `lifesteal:beacon_of_life` && !beaconDebounceMap.has(player.name) && !player.isSneaking) {
        reviveGuiForm(player);
        beaconMap.set(player.id, data2.block);
      }
    }
  });
});
...
world23.afterEvents.worldLoad.subscribe(() => {
  for (const scoreboard of scoreboards) {
    try {
      world23.scoreboard.addObjective(scoreboard);
    } catch {
    }
  }
  try {
    world23.scoreboard.addObjective(`hearts`, `\xA7cHearts`);
  } catch {
  }
  config.prefix = world23.getDynamicProperty(`lifesteal:prefix`) ?? config.prefix;
  ...
  Promise.resolve().then(() => init_ghostHandling());
  Promise.resolve().then(() => init_reviveSystem());
});
/*!
